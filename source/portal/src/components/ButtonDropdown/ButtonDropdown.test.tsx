/*
Copyright Amazon.com, Inc. or its affiliates. All Rights Reserved.

Licensed under the Apache License, Version 2.0 (the "License").
You may not use this file except in compliance with the License.
You may obtain a copy of the License at

    http://www.apache.org/licenses/LICENSE-2.0

Unless required by applicable law or agreed to in writing, software
distributed under the License is distributed on an "AS IS" BASIS,
WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
See the License for the specific language governing permissions and
limitations under the License.
*/
import React from "react";
import { render, fireEvent } from "@testing-library/react";
import Button, { ButtonProps } from "./ButtonDropdown";

jest.mock("react-i18next", () => ({
  useTranslation: () => {
    return {
      t: (key: any) => key,
      i18n: {
        changeLanguage: jest.fn(),
      },
    };
  },
  initReactI18next: {
    type: "3rdParty",
    init: jest.fn(),
  },
}));

describe("Button Component", () => {
  const defaultProps: ButtonProps = {
    items: [
      { text: "Item 1", id: "1" },
      { text: "Item 2", id: "2" },
    ],
    onItemClick: jest.fn(),
  };

  test("should render the button with text", () => {
    const { getByText } = render(<Button {...defaultProps}>Button</Button>);
    const buttonElement = getByText("Button");
    expect(buttonElement).toBeInTheDocument();
  });

  test("should toggle dropdown when button is clicked", () => {
    const { getByText, queryByText } = render(
      <Button {...defaultProps}>Button</Button>
    );
    const buttonElement = getByText("Button");
    fireEvent.click(buttonElement);

    const item1 = queryByText("Item 1");
    expect(item1).toBeInTheDocument();

    fireEvent.click(buttonElement);
    expect(queryByText("Item 1")).not.toBeInTheDocument();
  });

  test("should call onItemClick when dropdown item is clicked", () => {
    const { getByText } = render(<Button {...defaultProps}>Button</Button>);
    const buttonElement = getByText("Button");
    fireEvent.click(buttonElement);

    const item1 = getByText("Item 1");
    fireEvent.click(item1);

    expect(defaultProps.onItemClick).toHaveBeenCalledWith({
      text: "Item 1",
      id: "1",
    });
  });

  test("should display loading when loading prop is true", () => {
    const { container, queryByText } = render(
      <Button {...defaultProps} loading={true}>
        Button
      </Button>
    );
    expect(queryByText("button")).not.toBeInTheDocument();
    const elementWithTestClass = container.querySelector(".gsui-loading");
    expect(elementWithTestClass).toBeInTheDocument();
  });
});
