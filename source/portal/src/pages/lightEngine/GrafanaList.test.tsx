...
describe(__filename, () => {
  beforeEach(() => {
    // TODO: This is workaround for Error: Error: connect ECONNREFUSED 127.0.0.1:80. Fix this later.
  ...
  it("click remove", async () => {
    const result = renderWithProviders(<MemoryRouter initialEntries={["/grafana/import"]}>
      <GrafanaList />
    </MemoryRouter>, {
      preloadedState: {
        grafana: {
  ...
  it("click edit", async () => {
    ...
    await waitFor(() => {
      ...
      expect(result.getByText("b")).toBeTruthy();
      fireEvent.click(result.getByText("a"));
      fireEvent.click(result.getByText("button.edit"));
    })
    cleanup();
  });
