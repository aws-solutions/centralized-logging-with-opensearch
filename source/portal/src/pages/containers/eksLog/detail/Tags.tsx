...
interface TagProps {
  eksLogSourceInfo: any | undefined;
}

const Tags: React.FC<TagProps> = ({
  eksLogSourceInfo: eksLogInfo,
}: TagProps) => {
  ...
  return (
    <div>
      <HeaderPanel
        title={t("tag.name")}
        count={eksLogInfo?.tags?.length}
        contentNoPadding
      >
        {(eksLogInfo?.tags?.length || 0) > 0 ? (
          <div>
            ...
            {eksLogInfo?.tags?.map((tag: any, index: number) => {
              return (
