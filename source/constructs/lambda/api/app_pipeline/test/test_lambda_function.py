...
def test_create_light_engine_app_pipeline(mocker, ddb_client, sfn_client):
    from lambda_function import lambda_handler

    mocker.patch(
        "lambda_function.verify_s3_bucket_prefix_overlap_for_event_notifications",
        return_value=None,
    )
...
def test_get_scheduler_expression(events_client, scheduler_client):
    ...
    response = get_scheduler_expression(
        name=f"LogProcessor-{light_engine_app_pipeline_id}", group="default", client=events
    )
    ...
    response = get_scheduler_expression(
        name=f"LogMergerForMetrics-{light_engine_app_pipeline_id}", group="default", client=events
    )
    assert response == "cron(0 1 * * ? *)"

    response = get_scheduler_expression(
        name=f"LogArchive-{light_engine_app_pipeline_id}", group="default", client=events
    )
