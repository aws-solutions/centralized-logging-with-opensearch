...
import boto3
import pytest
from moto import (mock_dynamodb, mock_stepfunctions, mock_es, mock_ec2, mock_cloudwatch,
                  mock_sts)
...
def sfn_client():
    with mock_stepfunctions():
        ...
        response = sfn.create_state_machine(
            ...
            roleArn="arn:aws:iam::783732175206:role/LogHub-LogHubAPIClusterFlowSMSMRole",
        )
...
def ec2_client():
    with mock_ec2():
        ...
        subnet_resp = ec2.create_subnet(CidrBlock="18.0.128.0/20", VpcId=vpc_id, AvailabilityZone='us-west-2a')
        subnet_id = subnet_resp['Subnet']['SubnetId']

        sg_resp = ec2.create_security_group(Description="Test security group",
                                            GroupName="es-sg",
                                            VpcId=vpc_id)
...
def test_list_domain_names(sts_client, ddb_client, sfn_client, es_client):
    ...
    res = lambda_handler(make_graphql_lambda_event('listDomainNames', {'region': 'us-west-2'}),
                         None)
    assert res['domainNames'] == []


def test_list_imported_domains(sts_client, ddb_client, cw_client, sfn_client, es_client):
    import lambda_function
    res = lambda_function.lambda_handler(make_graphql_lambda_event('listImportedDomains',
                                                                   {'metrics': True}), None)
...
def test_get_domain_vpc(sts_client, ddb_client, sfn_client, es_client):
    ...
    with patch('boto3.client'):
        with patch('boto3.client.describe_elasticsearch_domain'):
            res = lambda_function.lambda_handler(make_graphql_lambda_event('getDomainVpc',
                                                                           {'region': 'us-west-2',
                                                                            'domainName': 'loghub-aos'}), None)
            assert res['vpcId']
...
def test_get_domain_details(sts_client, ddb_client, sfn_client, es_client):
    ...
    with patch('boto3.client'):
        with patch('boto3.client.describe_elasticsearch_domain'):
            res = lambda_function.lambda_handler(make_graphql_lambda_event('getDomainDetails',
                                                                           {'id': '40485c141648f8d0acbbec6eda19a4a7'}),
                                                 None)
...
def test_remove_domain(sts_client, sfn_client, ddb_client):
    ...
    with patch('boto3.client'):
        with patch('boto3.client.describe_elasticsearch_domain'):
            res = lambda_function.lambda_handler(make_graphql_lambda_event('removeDomain',
                                                                           {'id': '40485c141648f8d0acbbec6eda19a4a7'}),
                                                 None)
            assert res == 'OK'


def test_delete_proxy_for_open_search(sts_client, sfn_client, ddb_client):
    import lambda_function

    res = lambda_function.lambda_handler(make_graphql_lambda_event('deleteProxyForOpenSearch',
                                                                   {'id': '40485c141648f8d0acbbec6eda19a4a7'}), None)
    assert res == 'OK'


def test_create_proxy_for_open_search(sts_client, sfn_client, ddb_client):
    input = {
        ...
        "certificateArn": "arn:aws:acm:us-west-2:783732175206:certificate/525ca843-4f47-45c2-9e57-38979df03339",
        "keyName": "oregon"
    }

    import lambda_function
    res = lambda_function.lambda_handler(make_graphql_lambda_event('createProxyForOpenSearch',
                                                                   {'id': '40485c141648f8d0acbbec6eda19a4a7',
                                                                    'input': input, 'stack_type': 'Proxy'}), None)
    assert res == 'OK'
...
def test_delete_proxy_for_open_search(sts_client, sfn_client, ddb_client):
    ...
    res = lambda_function.lambda_handler(make_graphql_lambda_event('deleteAlarmForOpenSearch',
                                                                   {'id': '40485c141648f8d0acbbec6eda19a4a7','stack_type': 'Alarm'}), None)
    assert res == 'OK'


def test_create_proxy_for_open_search(sts_client, sfn_client, ddb_client):
    input = {
        "email": "taomat@amazon.com",
        "phone": "",
        "alarms": [
            {
                "type": "CLUSTER_RED",
    ...
    import lambda_function
    res = lambda_function.lambda_handler(make_graphql_lambda_event('createAlarmForOpenSearch',
                                                                   {'id': '40485c141648f8d0acbbec6eda19a4a7',
                                                                    'input': input, 'stack_type': 'Alarm'}), None)
    assert res == 'OK'
