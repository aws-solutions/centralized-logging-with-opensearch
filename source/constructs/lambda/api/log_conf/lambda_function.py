...
def check_time_format(**args):
    ...
    match = False
    try:
        res = datetime.strptime(time_srt, format_str.replace("%L", "%f"))
...
def create_log_conf(**args):
    ...
    log_conf_table.put_item(
        Item={
            ...
            "userSampleLog": args.get("userSampleLog", ""),
            ...
            "syslogParser": args.get("syslogParser"),
            "timeRegularExpression": args.get("timeRegularExpression", ""),
            "processorFilterRegex": _validate_filter_regex(
                args.get("processorFilterRegex", {"enable": False})
            ),
            "timeKey": args.get("timeKey"),
            "timeOffset": args.get("timeOffset"),
            "status": "ACTIVE",
        }
    )
    return id
...
def list_log_confs(page=1, count=20, confName=None, logType=None):
    ...
    response = log_conf_table.scan(
        ...
        ProjectionExpression="id, confName, logType, createdDt, #s ",
...
def update_log_conf(**args):
    """update confName in LogConf table"""
    logger.info("Update LogConf  in DynamoDB")
    conf_id = args["id"]
    conf_name = args["confName"]
    resp = log_conf_table.get_item(Key={"id": conf_id})
    if "Item" not in resp:
        raise APIException("Log Config Not Found")
    resp = list_log_confs(confName=conf_name)
    total = resp["total"]
    confs = resp["logConfs"]
    """Check if the confName exists """
    if total > 0 and confs[0]["id"] != conf_id:
        raise APIException("confName already exists")

    update_expr = (
        "SET confName=:cfn, logType=:lt, multilineLogParser=:multilineLogParser, "
        "syslogParser=:syslogParser, updatedDt= :uDt, userLogFormat=:userLogFormat, userSampleLog=:userSampleLog,"
        "regularExpression=:regularExpression,timeRegularExpression=:timeRegularExpression, regularSpecs=:regularSpecs,"
        "processorFilterRegex=:processorFilterRegex, timeKey=:timeKey, timeOffset=:timeOffset "
    )
    log_conf_table.update_item(
        Key={"id": conf_id},
...
def _validate_filter_regex(map: map) -> map:
    ...
    if not isinstance(map, dict):
        raise APIException("Invalid processorFilterRegex: item is not a dict or None.")
