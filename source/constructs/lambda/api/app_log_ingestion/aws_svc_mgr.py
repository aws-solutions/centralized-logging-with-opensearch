...
class SvcManager:
    ...
    def get_client(
        ...
        Args:
            ...
            region (_type_, optional): _description_. Defaults to default_region.
        ...
        if sub_account and sub_account_id != account_id:
            logger.info(f"subAccountRoleArn is {sub_account.get('subAccountRoleArn')}")
            ...
            if type.value == "client":
                return boto3.client(
                    service_name, region_name=region, config=default_config
                )
            else:
                return boto3.resource(
                    service_name, region_name=region, config=default_config
    ...
    def get_link_account(self, sub_account_id: str, region=default_region):
        ...
        if sub_account_id and sub_account_id != account_id:
            ...
            conditions = conditions.__and__(Attr("subAccountId").eq(sub_account_id))
            ...
            response = sub_account_link_table.scan(
                ...
                "subAccountKMSKeyArn, createdDt, #region ",
    ...
    def get_document_name(
        self, sub_account_id: str, type: DocumentType, region=default_region
    ) -> str:
        """Get the ssm document name for Fluent Bit

        Args:
            sub_account_id (str): linked account id
            type (DocumentType):  AGENT_INSTALL is the ssm document for Fluent-Bit installation,
                                AGENT_CONFIGURATION is the ssm document for Fluent-Bit configuration
    ...
    def get_sub_account_links(self, page=1, count=1000):
        """Get a list of sub account links"""
        logger.info(
