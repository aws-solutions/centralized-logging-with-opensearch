...
get_os_version() {
    ...
    if [ $(echo ${os_name} | grep -i ubuntu | wc -l) -ge 1 ]; then
        if [ $(echo ${os_version} | grep -i 20 | wc -l) -ge 1 ]; then
            echo ${OS_UBUNTU_20}
...
install_fluent_bit() {
    ...
    mkdir -p /opt
    download "https://${FLB_PKG_DOMAIN}/aws-for-fluent-bit%3A2.28.4/fluent-bit${suffix}.tar.gz" /opt/fluent-bit.tar.gz
    cd /opt
...
fetch_config_and_restart_fluent_bit() {
    ...
    aws s3 cp s3://${BUCKET_NAME}/app_log_config/${ASG_NAME}/uniform-time-format.lua /opt/fluent-bit/etc/
