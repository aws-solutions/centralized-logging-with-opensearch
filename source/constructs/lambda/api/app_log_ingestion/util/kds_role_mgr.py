...
class KDSRoleMgr():
    ...
    @staticmethod
    def generate_kds_role_4_fluent_bit(app_pipeline_id: str,
        ...
        #kdsRoleArn = app_pipeline.get('kdsRoleArn', '')
        role_arn = app_pipeline.get('kdsRoleArn') or app_pipeline.get(
            'ec2RoleArn')
        if role_arn:
            # exist, update assume role policy document
            role_name = app_pipeline.get('kdsRoleName') or app_pipeline.get(
                'ec2RoleName')
            ...
            if len(trust_account_set) > 0:
                ...
                KDSRoleMgr.validate_kds_role(roleArn=role_arn,
                                             ids=trust_account_set)
        else:
            # not exist
            logger.info(
                f"This application log pipeline({app_pipeline_id}) does not create a role for Kinesis Data Stream or AOS, please create it manually through Lambda."
            )

    @staticmethod
    def validate_kds_role(roleArn: str, ids=set()):
        ...
        for sub_account_id in ids:
            ...
            while tried < retry_limit:
                try:
                    sts_ca = svcMgr.get_client(sub_account_id=sub_account_id,
                                               service_name='sts',
        ...
        if not role_valid:
            logger.error(error)
            raise APIException(
                f"The role used to send logs to Kinesis Data Stream takes too long to be active. Please try again."
            )