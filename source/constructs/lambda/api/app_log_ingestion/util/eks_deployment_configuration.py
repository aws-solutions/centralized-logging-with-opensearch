...
class EKSClusterPodDeploymentConfigurationMng:
    ...
    def __init__(self, eks_cluster_id: str):
        ...
        if not self.__deployment_kind or (
                self.__deployment_kind != DEPLOYMENTKIND.DAEMONSET.value and self.__deployment_kind != DEPLOYMENTKIND.SIDECAR.value):
    ...
    def __get_sidecar_configuration_deploy_content(self) -> str:
        ...
        if app_log_ingestion["sourceId"] != self.__eks_cluster_id or app_log_ingestion["sourceType"] != SOURCETYPE.EKS_CLUSTER.value:
            raise APIException('EKS Source AppPipeline Not Found')
        ...
        eks_cluster_log_source = self.__log_ingestion_svc.get_eks_cluster_log_source(self.__eks_cluster_id)
        ...
        ns_and_svcacct_and_role = fluent_bit.generate_k8s_ns_and_svcacct_and_role(log_agent_role_arn)
        # configmap=fluent-bit-configmap.template+input_and_output_filter+user_define_parser
        inout_and_filter = fluent_bit.generate_k8s_fluent_bit_inout_and_filter(cri,self.__deployment_kind)
        user_define_parser = fluent_bit.generate_k8s_fluent_bit_user_defined_parser()
        configmap = fluent_bit.generate_k8s_configmap(inout_and_filter, '',user_define_parser,'')
    ...
    def __get_eks_cluster_ids_by_name(self,eks_cluster_name:str,eks_account_id:str,region_name:str)->list:
        """
        Get eks cluseter id array by name
        """
        eks_cluster_log_source_list=self.__log_ingestion_svc.get_eks_cluster_list_by_name(eks_cluster_name,eks_account_id,region_name)
        eks_cluster_ids=[]
    ...
    def __get_daemonset_configuration_deploy_content(self) -> str:
        ...
        #get eks cluseter id array by name
        eks_cluster_list=self.__get_eks_cluster_ids_by_name(eks_cluster_name,eks_account_id=eks_cluster_log_source['accountId'],region_name=eks_cluster_log_source['region'])
        if eks_cluster_list:
            resp = self.__log_ingestion_svc.list_app_log_ingestions(sourceId=self.__eks_cluster_id,
                                                                souceType=SOURCETYPE.EKS_CLUSTER.value, page=1,
                                                                count=1000)
                                                                
            if resp['total'] > 0:
                #traverse all ingestions under the same eks cluster
                resp = self.__log_ingestion_svc.list_app_log_ingestions(sourceIds=eks_cluster_list,
                                                                souceType=SOURCETYPE.EKS_CLUSTER.value, page=1,
                                                                count=1000)
                ...
                for app_log_ingestion in app_log_ingestions:
                    config_id=app_log_ingestion['confId']
                    fluent_bit = FluentBit.get_fluent_bit_instance(config_id,app_log_ingestion['appPipelineId'],app_log_ingestion['id'])
                    ...
                    # configmap=fluent-bit-configmap.template+input_and_output_filter+user_define_parser
                    inout_and_filter += fluent_bit.generate_k8s_fluent_bit_inout_and_filter(cri)
                ...
                if fluent_bit:
                    configmap = fluent_bit.generate_k8s_configmap(inout_and_filter,docker_firstline_parser,user_define_parser,user_define_multiline_parser)
                    daemonset_config = fluent_bit.generate_k8s_daemonset_config(eks_cluster_name)
