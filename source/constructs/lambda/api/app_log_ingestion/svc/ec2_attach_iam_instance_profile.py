...
def get_add_role_arn_from_instance_profile(iam_client, associate_instance_profile_arn: str) -> str:
...
def get_attach_policy_arn_from_role(iam_client, role_arn: str) -> str:
    ...
    policy_arn = ''
    for attach_policy in iam_client.list_attached_role_policies(RoleName=role_name)['AttachedPolicies']:
...
def get_ec2_iam_instance_profile_associate_status(ec2_client, instance_ids: list[str]) -> dict:
    ...
    for page_iterator in paginator.paginate(Filters=[
                                                {
                                                    'Name': 'instance-id',
                                                    'Values': instance_ids
                                                    },
        ...
        for association in page_iterator['IamInstanceProfileAssociations']:
            logger.info(f'Instance id: {association["InstanceId"]} already associate IAM instance Profile: {association["IamInstanceProfile"]["Arn"]}.')
            iam_instance_profile_arns.add(association['IamInstanceProfile']['Arn'])
            instance_ids.remove(association['InstanceId'])
    
    logger.info(f'Need to associate IAM instance profile of instance ids: {instance_ids}.')
    return dict(IamInstanceProfileArns=list(iam_instance_profile_arns), DisassociateInstanceIds=instance_ids)


def attach_permission_to_instance(instance_ids: list[str], associate_instance_profile_arn: str, sts_role_arn: str):
    ...
    logger.info(f'subAccountIamInstanceProfileArn: {associate_instance_profile_arn}, subAccountRoleArn: {sts_role_arn}')
    
    iam_client = conn.get_client('iam', sts_role_arn=sts_role_arn)
    ec2_client = conn.get_client('ec2', sts_role_arn=sts_role_arn)
    
    add_role_arn = get_add_role_arn_from_instance_profile(iam_client=iam_client, associate_instance_profile_arn=associate_instance_profile_arn)
    attach_policy_arn = get_attach_policy_arn_from_role(iam_client=iam_client, role_arn=add_role_arn)
    
    ec2_instance_profile_status = get_ec2_iam_instance_profile_associate_status(ec2_client=ec2_client, instance_ids=instance_ids)
    attach_policy_to_instance_profile_role(iam_client=iam_client, add_role_arn=add_role_arn, attach_policy_arn=attach_policy_arn, instance_profile_arns=ec2_instance_profile_status['IamInstanceProfileArns'])
    associate_iam_instance_profile(ec2_client=ec2_client, associate_instance_profile_arn=associate_instance_profile_arn, instance_ids=ec2_instance_profile_status['DisassociateInstanceIds'])
    
    
def attach_policy_to_instance_profile_role(iam_client, add_role_arn: str, attach_policy_arn: str, instance_profile_arns: list[str]):
    ...
    for instance_profile_arn in instance_profile_arns:
        ...
        if instance_profile_info['InstanceProfile']['Roles']:
            ...
            if need_attach_policy is True:
                ...
                iam_client.attach_role_policy(RoleName=role_name, PolicyArn=attach_policy_arn)
        else:
            logger.info(f'Add role {add_role_name} to {instance_profile_name}.')
            iam_client.add_role_to_instance_profile(InstanceProfileName=instance_profile_name, RoleName=add_role_name)


def associate_iam_instance_profile(ec2_client, associate_instance_profile_arn: str , instance_ids: list[str]):
    ...
    for instance_id in instance_ids:
        try:
            logger.info(f'Associate IAM instance profile {associate_instance_profile_arn} to instance id {instance_id}.')
            ec2_client.associate_iam_instance_profile(IamInstanceProfile={'Arn': associate_instance_profile_arn},
