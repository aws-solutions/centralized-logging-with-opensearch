...
eks_cluster_log_source_table_name = os.environ.get("EKS_CLUSTER_LOG_SOURCE_TABLE")
log_agent_eks_deployment_kind_table_name = os.environ.get("LOG_AGENT_EKS_DEPLOYMENT_KIND_TABLE")
...
log_agent_eks_deployment_kind_table = dynamodb.Table(log_agent_eks_deployment_kind_table_name)
...
eks_oidc_provider_arn_prefix = os.environ.get("EKS_OIDC_PROVIDER_ARN_PREFIX",
                                              f"arn:aws:iam::{account_id}:oidc-provider/")

daemonset_svc_account = os.environ.get("SVC_ACCOUNT", "system:serviceaccount:logging:fluent-bit")
sidecar_svc_account = os.environ.get("SVC_ACCOUNT", "system:serviceaccount:*:fluent-bit")
...
class IamManager():
    ...
    def create_iam_role(self, deployment_kind: str,oidc_provider_url: str, tags) -> str:
        self._deployment_kind=deployment_kind
        sys_tags = self.convert_tag(tags)
        eks_log_agent_role = f'LogHub-EKS-LogAgent-Role-{str(uuid.uuid4()).replace("-", "")}'
        oidc_provider_arn = self.generate_oidc_provider(oidc_provider_url, sys_tags)
        policy_document = self.generate_assume_role_policy_document(oidc_provider_arn)
        response = self.iam.create_role(RoleName=eks_log_agent_role, AssumeRolePolicyDocument=policy_document,
                                        Description='Using this role to associate EKS svc account', Tags=sys_tags)
        return response['Role']['Arn']

    def generate_oidc_provider(self, oidc_provider_url: str, tags) -> str:
        """
        arn:aws:iam::1234567890:oidc-provider/oidc.eks.region.amazonaws.com/id/oidc_issuer
        """
        oidc_provider_arn = f'{eks_oidc_provider_arn_prefix}{oidc_provider_url.replace("https://", "")}'
        need_create_open_id_connect_provider = False
        try:
            oidc_response = self.iam.get_open_id_connect_provider(OpenIDConnectProviderArn=oidc_provider_arn)
    ...
    def generate_assume_role_policy_document(self, oidc_provider_arn: str):
        ...
        if self._deployment_kind==DEPLOYMENTKIND.DAEMONSET.value:
            _assume_role_policy_document_template_path = './role_template/assume_role_policy_document_daemonset.template'
            svc_account=daemonset_svc_account
        else:
            _assume_role_policy_document_template_path = './role_template/assume_role_policy_document_sidecar.template'
            svc_account=sidecar_svc_account

        policy_document = ""
        
        provider = oidc_provider_arn.replace(eks_oidc_provider_arn_prefix, "")

        with open(_assume_role_policy_document_template_path, "r") as openFile:
            for line in openFile:
                line = line.replace("$OIDC_PROVIDER_ARN", oidc_provider_arn) \
                    .replace("$PROVIDER", provider) \
                    .replace("$OIDC_CLIENT_ID", eks_oidc_client_id) \
                    .replace("$SVC_ACCOUNT", svc_account)
...
class EksClusterManager:
    ...
    def list_eks_cluster_names(self, nextToken: str, isListAll=False):
        ...
        eks = boto3.client('eks', config=default_config)
    ...
    def import_eks_cluster(self, **args) -> str:
        ...
        eks_account_id = args.get("accountId", account_id)
        cri = args.get("cri", '')
        ...
        if self.is_exist(eks_cluster_name, args.get("aosDomainId"), eks_account_id, region_name):
            raise APIException(f'EKS Cluster "{eks_cluster_name}" has already imported')
        ...
        if ("DELETING" in eks_cluster['cluster']["status"]) or ("FAILED" in eks_cluster['cluster']["status"]):
        ...
        logger.info(f'eks "{eks_cluster_name}" describe cluster reponse is {eks_cluster}')
        ...
        cluster_security_group_id = eks_cluster['cluster']['resourcesVpcConfig'].get('clusterSecurityGroupId')
        oidc_issuer = eks_cluster['cluster']['identity']['oidc']['issuer']
        endpoint = eks_cluster['cluster']['endpoint']
        tags = args.get("tags", [])

        agent_role_arn = self.get_eks_cluster_log_agent_role_arn_by_name(eks_cluster_name, eks_account_id, region_name)
        if not agent_role_arn:
            # create IAM Role
            iam_mgr = IamManager()
            agent_role_arn = iam_mgr.create_iam_role(deployment_kind,oidc_issuer, tags)
    ...
    def remove_eks_cluster(self, id) -> str:
        ...
        conditions = conditions.__and__(Attr('sourceType').eq(SOURCETYPE.EKS_CLUSTER.value))
    ...
    def get_eks_cluster_details(self, eksClusterId) -> str:
        ...
        resp['Item']['deploymentKind'] = self.get_log_agent_eks_deployment_kind_by_id(eks_cluster_id=eksClusterId)

        # get AOS domain by aosDomainId
        aos_domain_id = resp['Item']['aosDomainId']
        resp['Item']['aosDomain'] = self.get_aos_imported_domain_by_id(aos_domain_id)

        return resp['Item']

    def is_exist(self, eks_cluster_name: str, aos_domain_id: str, eks_account_id: str, region_name: str) -> bool:
        """
    ...
    def get_eks_cluster_log_agent_role_arn_by_name(self, eks_cluster_name: str, eks_account_id: str, region_name: str) -> str:
        """
    ...
    def list_imported_eks_clusters(self, page=1, count=20):
        ...
        List eks clusters 
        """
        logger.info(f'List eks cluster log source from DynamoDB in page {page} with {count} of records')
        ...
        for eks_cluster in eks_clusters:
            eks_cluster['deploymentKind'] = self.get_log_agent_eks_deployment_kind_by_id(
                eks_cluster_id=eks_cluster['id'])
            eks_cluster['aosDomain'] = self.get_aos_imported_domain_by_id(aos_domain_id=eks_cluster['aosDomainId'])
    ...
    def get_log_agent_eks_deployment_kind_by_id(self, eks_cluster_id: str) -> str:
        """
        get deployment kind by eks cluster id
        """
        conditions = Attr('eksClusterId').eq(eks_cluster_id)
