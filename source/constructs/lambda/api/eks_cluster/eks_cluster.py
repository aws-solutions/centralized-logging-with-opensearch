...
EKS_NOT_FOUND_ERROR = "EKS Cluster Not Found"
...
eks_oidc_provider_arn_prefix = os.environ.get(
    ...
    f"arn:{partition}:iam::{default_account_id}:oidc-provider/",
)
daemonset_svc_account = os.environ.get(
    "SVC_ACCOUNT", "system:serviceaccount:logging:fluent-bit"
)
sidecar_svc_account = os.environ.get(
    "SVC_ACCOUNT", "system:serviceaccount:*:fluent-bit"
)
...
class IamManager:
    ...
    def __init__(self, sub_acct_id: str, region: str):
        ...
        if sub_acct_id == default_account_id:
    ...
    def create_iam_role(
        self, deployment_kind: str, oidc_provider_url: str, tags
    ) -> str:
        self._deployment_kind = deployment_kind
        sys_tags = self.convert_tag(tags)
        eks_log_agent_role = (
            f'{stack_prefix}-EKS-LogAgent-Role-{str(uuid.uuid4()).replace("-", "")}'
        )
    ...
    def generate_assume_role_policy_document(self, oidc_provider_arn: str):
        ...
        provider = oidc_provider_arn.replace(self.__eks_oidc_provider_arn_prefix, "")

        with open(_assume_role_policy_document_template_path, "r") as openFile:
            for line in openFile:
                line = (
                    line.replace("$OIDC_PROVIDER_ARN", oidc_provider_arn)
...
class EksClusterManager:
    ...
    def list_eks_cluster_names(self, **args):
        ...
        list_all = args.get("isListAll", False)
        ...
        if list_all:
            include = ["all"]
        resp = eks.list_clusters(include=include, nextToken=next_token)
        ...
        result = [
            item for item in resp.get("clusters") if item not in imported_clusters
        ]
    ...
    def import_eks_cluster(self, **args) -> str:
        ...
        eks_account_id = args.get("accountId") or default_account_id
        cri = args.get("cri", "")
        ...
        if self.is_exist(
            eks_cluster_name, args.get("aosDomainId"), eks_account_id, region_name
        ):
        ...
        try:
            ...
            if e.response["Error"]["Code"] == "ResourceNotFoundException":
                raise APIException(EKS_NOT_FOUND_ERROR)
            else:
                raise e
        ...
        cluster_security_group_id = eks_cluster["cluster"]["resourcesVpcConfig"].get(
    ...
    def remove_eks_cluster(self, id) -> str:
        ...
        response = app_log_ingestion_table.scan(
            FilterExpression=conditions,
            ProjectionExpression="id,#s,sourceType,sourceId ",
    ...
    def get_eks_cluster_details(self, **args) -> str:
        """
        get EKS Cluster details
        """
        eks_cluster_id = args.get("eksClusterId", "")
        resp = eks_cluster_log_source_table.get_item(Key={"id": eks_cluster_id})
    ...
    def get_eks_cluster_log_agent_role_arn_by_name(
        ...
        response = eks_cluster_log_source_table.scan(
            FilterExpression=conditions,
            ProjectionExpression="id,logAgentRoleArn,#s",
            ExpressionAttributeNames={
                "#s": "status",
            },
        )
