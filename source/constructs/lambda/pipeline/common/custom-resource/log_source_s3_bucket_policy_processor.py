...
log_source_region = os.environ.get("LOG_SOURCE_REGION",
                                   default_region) or default_region
...
def on_create():
    if account_id != log_source_account_id and log_source_account_id and log_type not in [
            "RDS", "Lambda"
    ]:
        try:
            s3 = get_client('s3')
            ...
            resp = s3.put_bucket_notification_configuration(
                ...
                NotificationConfiguration={
                    ...
                    'LambdaFunctionConfigurations': history_config.get("LambdaFunctionConfigurations", []),
                })
            logger.info(
                f'put_bucket_notification_configuration resp is {resp}')
...
def on_delete():
    if account_id != log_source_account_id and log_source_account_id and log_type not in [
        ...
        try:
            ...
            logger.info(f'history notification config is {history_config}')
            queue_configurations = history_config.get("QueueConfigurations", [])
            deleted_queue_configurations = [x for x in queue_configurations if x["Id"] != notification_id]
