...
import * as LambdaWorker from './LambdaWorker';
import { httpsRequest, buildRequest } from '../../common/common'

const _region = process.env.AWS_REGION;

/**
 * Worker for sending Lambda logs to OpenSearch
 * @param records 
 * @param endpoint 
 * @param context 
 * @returns 
 */
async function sendLambda(records: any, endpoint: any, context: any) {
    for (const record of records) {
        ...
        if (awslogsData.messageType === "CONTROL_MESSAGE") {
            ...
            var openSearchBulkData = await LambdaWorker.transform(awslogsData);

            var requestParams = await buildRequest(endpoint, openSearchBulkData, _region!);
            
            try {
                const cwl_index = awslogsData.logStream + '_' + awslogsData.logEvents.timestamp
