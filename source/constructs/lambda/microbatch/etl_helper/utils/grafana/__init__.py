...
class GrafanaClient:
    ...
    def __init__(self, url: str, token: str, verify=True, timeout=30.0):
        ...
        self.api = GrafanaApi(
            url=self.url, token=self.token, verify=self.verify, timeout=self.timeout
        )
    ...
    def has_installed_athena_plugin(self) -> bool:
        if (
            self.plugin.get_plugin_settings(plugin_id="grafana-athena-datasource")[
                "status"
            ]
            == 200
        ):
    ...
    def check_folder_permission(self):
        ...
        self.folder.create_folder(
            title="TestFolderPermission", folder_uid=test_folder_uid
        )
    ...
    def check_permission(self) -> None:
        ...
        if self.check_url_connectivity() is False:
            raise ConnectionError(
                f"Failed to check url connectivity, grafana address: {self.url} cannot be accessed."
            )
    ...
    def update_datasource_by_name(self, datasource_name: str, datasource: dict):
        ...
        datasource_info = self.datasource.get_datasource_by_name(
            datasource_name=datasource_name
        )

        if datasource_info["status"] == 200:
            response = self.datasource.update_datasource(
                datasource_uid=datasource_info["content"]["uid"], datasource=datasource
            )
        else:
    ...
    def update_athena_connection_uid(
        ...
        Returns:
            str: _description_
        """
        datasource_info = self.create_athena_datasource(
    ...
    def create_athena_datasource(self, athena_connection: AthenaConnection) -> dict:
        ...
        return self.update_datasource_by_name(
            datasource_name=athena_connection.name, datasource=datasource
        )

    @staticmethod
    def replace_athena_connection_args_in_templates(
        dashboard: dict, athena_connection_args: dict
    ) -> dict:
        ...
        for template in dashboard.get("templating", {}).get("list", []):
            if (
                template.get("datasource", {}).get("type")
                == "grafana-athena-datasource"
            ):
                if (
                    template.get("query", {}).get("refId")
                    not in athena_connection_args.keys()
                ):
                    continue
                ...
                if not template["query"].get("connectionArgs"):
                    continue
                template["query"]["connectionArgs"][
                    "catalog"
                ] = athena_connection.catalog
                template["query"]["connectionArgs"][
                    "database"
                ] = athena_connection.database
                template["query"]["connectionArgs"]["region"] = athena_connection.region
                template["query"]["connectionArgs"][
                    "resultReuseEnabled"
                ] = athena_connection.reuse
                template["query"]["connectionArgs"][
                    "resultReuseMaxAgeInMinutes"
                ] = athena_connection.reuse_age

        return dashboard

    @staticmethod
    def replace_athena_connection_args_in_panels(
        dashboard: dict, athena_connection_args: dict
    ) -> dict:
        ...
        for panel in dashboard.get("panels", []):
            for target in panel.get("targets", []):
                if (
                    target.get("refId") in athena_connection_args.keys()
                    and target.get("datasource", {}).get("type")
                    == "grafana-athena-datasource"
                ):
    ...
    def import_dashboard(
        ...
        dashboard = self.replace_athena_connection_args_in_panels(
            dashboard=dashboard, athena_connection_args=athena_connection_args
        )
        dashboard = self.replace_athena_connection_args_in_templates(
            dashboard=dashboard, athena_connection_args=athena_connection_args
        )
        ...
        if (
            self.dashboard.get_dashboard(dashboard_uid=dashboard["uid"])["status"]
            != 200
...
class GrafanaApi:
    ...
    def _request(self, path: str, method, json: Union[str, None] = None) -> dict:
        ...
        response = self.api.request(
            method=method, url=url, json=json, verify=self.verify, timeout=self.timeout
        )
        ...
        try:
            ...
            logger.error(
                f"Grafana request failed, the response is {response.text}, not a json object."
            )
            r["status"] = 400
        except Exception as e:
            logger.error(
                (
                    f"Grafana request failed, the response is {response.text}, error message is {e}."
                )
            )
            r["status"] = 400
