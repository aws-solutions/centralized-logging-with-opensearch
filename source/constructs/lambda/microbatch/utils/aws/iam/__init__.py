...
class IAMClient:
    ...
    def get_policy_document(
        self, arn: str, sid: Union[str, None] = None, exclusive: list[str] = []
    ) -> dict:
    ...
    def update_policy_document(
        self, arn: str, sid: str, policy_document: dict, tries: int = 3
    ) -> dict:
        ...
        while tries:
            ...
            logger.warning(
                f"Document does not exist in Policy, retrying in {delay} seconds..."
            )
            time.sleep(delay)
            tries -= 1
    ...
    def delete_oldest_policy_version(self, arn: str) -> None:
        ...
        version_ids = [
            int(document["VersionId"][1:])
            for document in policy_versions["Versions"]
            if document["IsDefaultVersion"] is False
        ]
        for version_id in sorted(version_ids):
            try:
                self._iam_client.delete_policy_version(
                    PolicyArn=arn, VersionId=f"v{version_id}"
                )
                break
            except Exception as e:
                logger.error(e)
    ...
    def delete_policy_document(self, arn: str, sid: str, tries: int = 3) -> dict:
        ...
        while tries:
            ...
            delay = random.uniform(0.0, 3.0)
            logger.warning(
                f"Document still exists in Policy, retrying in {delay} seconds..."
            )
            time.sleep(delay)
            tries -= 1
    ...
    def get_role_policy(self, role_name: str, policy_name: str) -> dict:
        ...
        """
        try:
            response = self._iam_client.get_role_policy(
    ...
    def delete_role_policy(self, role_name: str, policy_name: str) -> dict:
        ...
        """
        if self.get_role_policy(role_name=role_name, policy_name=policy_name):
            return self._iam_client.delete_role_policy(
                RoleName=role_name, PolicyName=policy_name
            )
        return {}
    ...
    def add_service_principal_to_assume_role_policy(
        self, role_name: str, service_principal: str, tries: int = 3
    ) -> dict:
        ...
        while tries:
            try:
                ...
                assume_role_policy_document = role_info["Role"][
                ...
                response = self.update_assume_role_policy(
                    role_name=role_name, policy_document=assume_role_policy_document
                )
                exception = None
            except Exception as e:
                exception = e
                ...
                logger.warning(
                    f"Add service principal to assume role policy failed, retrying in {delay} seconds..."
                )
                time.sleep(delay)
                tries -= 1
