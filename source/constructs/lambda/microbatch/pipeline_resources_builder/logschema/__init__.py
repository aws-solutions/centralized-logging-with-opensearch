...
__all__ = [
    ...
    "RDS_PARQUET",
    "RDS_METRICS",
    "RDS_GRAFANA_DASHBOARD",
    "RDS_GRAFANA_DETAILS",
    "S3_RAW",
    "S3_PARQUET",
    "S3_METRICS",
    "S3_GRAFANA_DASHBOARD",
    "S3_GRAFANA_DETAILS",
]
...
# Application
APPLICATION_GRAFANA_DETAILS = json.load(
    open(f"{__CURRENT_PATH__}/application/details.json", "r")
)

# CloudFront
CLOUDFRONT_RAW_SCHEMA = json.load(open(f"{__CURRENT_PATH__}/cloudfront/raw.json", "r"))
CLOUDFRONT_PARQUET_SCHEMA = json.load(
    open(f"{__CURRENT_PATH__}/cloudfront/parquet.json", "r")
)
CLOUDFRONT_METRICS_SCHEMA = json.load(
    open(f"{__CURRENT_PATH__}/cloudfront/metrics.json", "r")
)
CLOUDFRONT_GRAFANA_DASHBOARD = json.load(
    open(f"{__CURRENT_PATH__}/cloudfront/dashboards.json", "r")
)
CLOUDFRONT_GRAFANA_DETAILS = json.load(
    open(f"{__CURRENT_PATH__}/cloudfront/details.json", "r")
)

CLOUDFRONT_RAW = TableMetaData(
    data_format=Tsv,
    schema=CLOUDFRONT_RAW_SCHEMA,
    table_properties={"skip.header.line.count": "2"},
    serialization_properties={"field.delim": "\t", "line.delim": "\n"},
    ignore_partition=True,
)
CLOUDFRONT_PARQUET = TableMetaData(
    data_format=Parquet, schema=CLOUDFRONT_PARQUET_SCHEMA, ignore_partition=False
)
CLOUDFRONT_METRICS = TableMetaData(
    data_format=Parquet, schema=CLOUDFRONT_METRICS_SCHEMA, ignore_partition=False
)
...
ALB_RAW = TableMetaData(
    data_format=Regex,
    schema=ALB_RAW_SCHEMA,
    serialization_properties={
        "input.regex": '^([^ ]*) ([^ ]*) ([^ ]*) ([^ ]*):([0-9]*) ([^ ]*)[:-]([0-9]*) ([-.0-9]*) ([-.0-9]*) ([-.0-9]*) (|[-0-9]*) (-|[-0-9]*) ([-0-9]*) ([-0-9]*) "([^ ]*) (.*) (- |[^ ]*)" "([^"]*)" ([A-Z0-9-_]+) ([A-Za-z0-9.-]*) ([^ ]*) "([^"]*)" "([^"]*)" "([^"]*)" ([-.0-9]*) ([^ ]*) "([^"]*)" "([^"]*)" "([^ ]*)" "([^ ]+?)" "([^ ]+)" "([^ ]*)" "([^ ]*)" ?(TID_[A-Za-z0-9.-]+)? ?(?:[^\\u007B]+?)? ?(\\u007B.*\\u007D)?$'
    },
    ignore_partition=True,
)
ALB_PARQUET = TableMetaData(
    data_format=Parquet, schema=ALB_PARQUET_SCHEMA, ignore_partition=False
)
ALB_METRICS = TableMetaData(
    data_format=Parquet, schema=ALB_METRICS_SCHEMA, ignore_partition=False
)
...
WAF_PARQUET = TableMetaData(
    data_format=Parquet, schema=WAF_PARQUET_SCHEMA, ignore_partition=False
)
WAF_METRICS = TableMetaData(
    data_format=Parquet, schema=WAF_METRICS_SCHEMA, ignore_partition=False
)

# CloudTrail
CLOUDTRAIL_RAW_SCHEMA = json.load(open(f"{__CURRENT_PATH__}/cloudtrail/raw.json", "r"))
CLOUDTRAIL_PARQUET_SCHEMA = json.load(
    open(f"{__CURRENT_PATH__}/cloudtrail/parquet.json", "r")
)
CLOUDTRAIL_METRICS_SCHEMA = json.load(
    open(f"{__CURRENT_PATH__}/cloudtrail/metrics.json", "r")
)
CLOUDTRAIL_GRAFANA_DASHBOARD = json.load(
    open(f"{__CURRENT_PATH__}/cloudtrail/dashboards.json", "r")
)
CLOUDTRAIL_GRAFANA_DETAILS = json.load(
    open(f"{__CURRENT_PATH__}/cloudtrail/details.json", "r")
)

CLOUDTRAIL_RAW = TableMetaData(
    data_format=CloudTrailLogs, schema=CLOUDTRAIL_RAW_SCHEMA, ignore_partition=True
)
CLOUDTRAIL_PARQUET = TableMetaData(
    data_format=Parquet, schema=CLOUDTRAIL_PARQUET_SCHEMA, ignore_partition=False
)
CLOUDTRAIL_METRICS = TableMetaData(
    data_format=Parquet, schema=CLOUDTRAIL_METRICS_SCHEMA, ignore_partition=False
)

# VPC Flow
VPCFLOW_RAW_SCHEMA = json.load(open(f"{__CURRENT_PATH__}/vpcflow/raw.json", "r"))
VPCFLOW_PARQUET_SCHEMA = json.load(
    open(f"{__CURRENT_PATH__}/vpcflow/parquet.json", "r")
)
VPCFLOW_METRICS_SCHEMA = json.load(
    open(f"{__CURRENT_PATH__}/vpcflow/metrics.json", "r")
)
VPCFLOW_GRAFANA_DASHBOARD = json.load(
    open(f"{__CURRENT_PATH__}/vpcflow/dashboards.json", "r")
)
VPCFLOW_GRAFANA_DETAILS = json.load(
    open(f"{__CURRENT_PATH__}/vpcflow/details.json", "r")
)

VPCFLOW_RAW = TableMetaData(
    data_format=Tsv,
    schema=VPCFLOW_RAW_SCHEMA,
    table_properties={"skip.header.line.count": "1"},
    serialization_properties={"field.delim": " ", "line.delim": "\n"},
    ignore_partition=True,
)
VPCFLOW_PARQUET = TableMetaData(
    data_format=Parquet, schema=VPCFLOW_PARQUET_SCHEMA, ignore_partition=False
)
VPCFLOW_METRICS = TableMetaData(
    data_format=Parquet, schema=VPCFLOW_METRICS_SCHEMA, ignore_partition=False
)
...
RDS_PARQUET = TableMetaData(
    data_format=Parquet, schema=RDS_PARQUET_SCHEMA, ignore_partition=False
)
RDS_METRICS = TableMetaData(
    data_format=Parquet, schema=RDS_METRICS_SCHEMA, ignore_partition=False
)
...
S3_RAW = TableMetaData(
    data_format=Regex,
    schema=S3_RAW_SCHEMA,
    serialization_properties={
        "input.regex": '^([^ ]*) ([^ ]*) \\[(.*?)\\] ([^ ]*) ([^ ]*) ([^ ]*) ([^ ]*) ([^ ]*) ("[^"]*"|-) (-|[0-9]*) ([^ ]*) ([^ ]*) ([^ ]*) ([^ ]*) ([^ ]*) ([^ ]*) ("[^"]*"|-) ([^ ]*)(?: ([^ ]*) ([^ ]*) ([^ ]*) ([^ ]*) ([^ ]*) ([^ ]*) ([^ ]*) (-|Yes))?.*$'
    },
    ignore_partition=True,
)
S3_PARQUET = TableMetaData(
    data_format=Parquet, schema=S3_PARQUET_SCHEMA, ignore_partition=False
)
S3_METRICS = TableMetaData(
    data_format=Parquet, schema=S3_METRICS_SCHEMA, ignore_partition=False
)
...
SES_PARQUET = TableMetaData(
    data_format=Parquet, schema=SES_PARQUET_SCHEMA, ignore_partition=False
)
SES_METRICS = TableMetaData(
    data_format=Parquet, schema=SES_METRICS_SCHEMA, ignore_partition=False
)
...
SOURCE_TYPE_RESOURCES = {
    ...
    "fluent-bit": {"dashboards": (APPLICATION_GRAFANA_DETAILS, None)},
    "s3": {"dashboards": (APPLICATION_GRAFANA_DETAILS, None)},
}
