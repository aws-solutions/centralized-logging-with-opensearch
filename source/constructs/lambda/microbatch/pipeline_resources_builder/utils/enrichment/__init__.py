...
class CloudFront(object):
    ...
    @cached_property
    def ip_address(self) -> str:
        return self.record.split(self.field_delimiter)[4] if len(self.record.split(self.field_delimiter)) >= 5 else ''
    
    @cached_property
    def user_agent(self) -> str:
        return self.record.split(self.field_delimiter)[10] if len(self.record.split(self.field_delimiter)) >= 11 else ''
...
class EnrichProcessor:
    ...
    def __init__(self, source_type: str):
        if source_type not in self.source_parser_mapping.keys():
            raise ValueError(f'Do not supported source type: {source_type}. Supported source type: {list(self.source_parser_mapping.keys())}.')
        
        self.source_type = source_type
        self.source_parser_cls = self.source_parser_mapping[source_type]
    
    def get_maxminddb_path(self) -> str:
        if os.environ.get('ENV', 'LAMBDA') == 'LAMBDA':
            return '/opt/python/maxminddb/GeoLite2-City.mmdb'
        return f'{os.path.dirname(os.path.abspath(__file__))}/maxminddb/GeoLite2-City.mmdb'
        ...
        try:
            ...
            if record and isinstance(record, dict):
                if 'country' in record:
                    enriched_data['geo_iso_code'] = record['country'].get('iso_code', '')
                    enriched_data['geo_country'] = record['country'].get('names', {}).get('en')
                    if 'city' in record:
                        enriched_data['geo_city'] = record['city'].get('names', {}).get('en')
                if 'location' in record:
                    enriched_data['geo_location'] = f"{record['location'].get('latitude')},{record['location'].get('longitude')}"
        except Exception as e:
            logger.error(f'Get the record for {cls.ip_address} from MaxMind DB failed, the error message: {e}')
