...
class SQSClient:
    ...
    def receive_message(
        self, url: str, attribute_names: list = ["ALL"], max_num: int = 10
    ) -> Iterator[dict]:
        logger.info(f"Receiving message from SQS {url}.")
        for message in self._sqs_client.receive_message(
            QueueUrl=url, AttributeNames=attribute_names, MaxNumberOfMessages=max_num
        ).get("Messages", []):
    ...
    def get_queue_policy(
        self, url: str, sid: Union[str, None] = None, exclusive: list[str] = []
    ) -> dict:
    ...
    def update_queue_policy(
        ...
        while tries:
            ...
            if self.get_queue_policy(url=queue_url, sid=sid)["Statement"]:
                logger.info(
                    f"New Statement already exists in Queue Policy Document, continuing."
                )
                exception = None
                break
            ...
            logger.warning(
                f"New Statement does not exists in Queue Policy Document, retrying in {delay} seconds..."
            )
            time.sleep(delay)
            tries -= 1
    ...
    def delete_queue_policy(self, arn: str, sid: str, tries: int = 3) -> dict:
        ...
        while tries:
            ...
            try:
                self._sqs_client.set_queue_attributes(
                    QueueUrl=queue_url, Attributes={"Policy": queue_policy_str}
                )
            except Exception as e:
                exception = e
                logger.warning(e)
            ...
            if not self.get_queue_policy(url=queue_url, sid=sid)["Statement"]:
                logger.info(
                    f"Statement no longer exists in Queue Policy Document, continuing."
                )
                exception = None
                break
            ...
            delay = random.uniform(0.0, 3.0)
            logger.warning(
                f"Statement still exists in Queue Policy Document, retrying in {delay} seconds..."
            )
            time.sleep(delay)
            tries -= 1
