...
class Schema:
    ...
    def char(length: int) -> DataType:
        if length <=0 or length > 255:
            raise ValueError(f'char length must be (inclusively) between 1 and 255, but was {length}.')
        if length % 1 != 0:
    ...
    def varchar(length: int) -> DataType:
        if length <=0 or length > 65535:
            raise ValueError(f'char length must be (inclusively) between 1 and 65535, but was {length}.')
        if length % 1 != 0:
            raise ValueError(f'char length must be a positive integer, was {length}.')
    ...
    def array(item_type: DataType) -> DataType:
        return DataType(is_primitive=False, input_string=f'array<{item_type.input_string}>')
    
    @staticmethod
    def map(key_type: DataType, value_type: DataType):
        if not key_type.is_primitive:
            raise TypeError(f"the key type of a 'map' must be a primitive, but was {key_type.input_string}")
        return DataType(is_primitive=False, input_string=f"map<{key_type.input_string},{value_type.input_string}>")
    
    @staticmethod
    def struct(columns: list):
        struct = ','.join([f'{column["name"]}:{column["type"].input_string}' for column in columns])
