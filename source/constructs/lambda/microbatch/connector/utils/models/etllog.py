...
class ETLLogTable:
    ...
    def delete(self, execution_name, task_id) -> None:
        return self.ddb_client.delete_item(
            key={self.model.partition_key: execution_name, self.model.sort_key: task_id}
        )

    def batch_delete(self, execution_name) -> None:
        for item in self.query_item(execution_name=execution_name):
            self.delete(
                execution_name=item[self.model.partition_key],
                task_id=item[self.model.sort_key],
            )
    ...
    def update(self, execution_name: str, task_id: str, item: dict) -> None:
        ...
        return self.ddb_client.update_item(
            key={
                self.model.partition_key: execution_name,
                self.model.sort_key: task_id,
            },
            item=item,
        )

    def get(
        self, execution_name: str, task_id: str, raise_if_not_found: bool = False
    ) -> dict:
    ...
    def query_item(
        ...
        for page_iterator in self.ddb_client.query(
            key_condition=Key(self.model.partition_key).eq(execution_name),
            filter=filter,
    ...
    def query_subtasks(
        self, execution_name: str, parent_task_id, consistent: bool = True
    ) -> Iterator[dict]:
        conditions = Attr(self.model.parent_task_id).eq(parent_task_id)
        for item in self.query_item(
            execution_name=execution_name, filter=conditions, consistent=consistent
        ):
            yield item

    def get_subtask_status_count(
        self,
        execution_name: str,
        parent_task_id: str,
        ...
        conditions = (
            Attr(self.model.parent_task_id)
            .eq(parent_task_id)
            .__and__(Attr(self.model.status).eq(status))
        )
        subtask_status_count = self.query_count(
            execution_name=execution_name, filter=conditions, consistent=consistent
        )
        parent_task_info = self.get(
            execution_name=execution_name, task_id=parent_task_id
        )
        total_subtasks = 0
        if parent_task_info is not None:
            total_subtasks = json.loads(parent_task_info[self.model.data]).get(
                "totalSubTask"
            )

        return {
            self.model.partition_key: execution_name,
            self.model.parent_task_id: parent_task_id,
            self.model.status: status,
            "taskCount": subtask_status_count,
