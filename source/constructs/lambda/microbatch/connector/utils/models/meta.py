...
class MetaTable:
    ...
    def check_shared_table_name_in_pipeline(self, meta_name: str) -> bool:
        ...
        if pipeline_info:
            table_name = pipeline_info.get('data', {}).get('destination', {}).get('table', {}).get('name', '')
        else:
            return has_duplicate_table
        
        conditions = Attr('type').eq('Pipeline').__and__(Attr(self.model.partition_key).ne(meta_name))
        for metadata in self.scan_item(filter=conditions, consistent=True):
            tmp_table_name = metadata.get('data', {}).get('destination', {}).get('table', {}).get('name', '')
    ...
    def check_shared_database_location_in_pipeline(self, meta_name: str) -> bool:
        ...
        pipeline_info = self.get(meta_name=meta_name)
        if pipeline_info:
            database_name = pipeline_info.get('data', {}).get('destination', {}).get('database', {}).get('name', '')
            location = pipeline_info.get('data', {}).get('destination', {}).get('location', {})
        else:
            return has_duplicate_table
        
        conditions = Attr('type').eq('Pipeline').__and__(Attr(self.model.partition_key).ne(meta_name))
        for metadata in self.scan_item(filter=conditions, consistent=True):
            tmp_database_name = metadata.get('data', {}).get('destination', {}).get('database', {}).get('name', '')
            tmp_location = pipeline_info.get('data', {}).get('destination', {}).get('location', {})
