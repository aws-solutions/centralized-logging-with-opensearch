...
class TestSchema:
    ...
    def test_schema_map(self):
        ...
        map_field = self.schema.map(key_type=self.schema.string(), value_type=self.schema.struct([{'name': 'year', 'type': self.schema.string()}, {'name': 'month', 'type': self.schema.integer()}]))
...
class TestTableMetaData:
    ...
    def test_application_parquet(self,):
        ...
        APPLICATION_PARQUET = TableMetaData(data_format=Parquet, schema=schema, ignore_partition=False)
    ...
    def test_application_regex(self,):
        ...
        APPLICATION_RAW = TableMetaData(data_format=Regex, schema=schema, serialization_properties={'input.regex': '(?<remote_addr>\\S+)\\s+-\\s+(?<remote_user>\\S+)\\s+\\[(?<time_local>\\d+\\S+\\d+:\\d+:\\d+:\\d+)\\s+\\S+\\]\\s+\'(?<request_method>\\S+)\\s+(?<request_uri>\\S+)\\s+\\S+\'\\s+(?<status>\\S+)\\s+(?<body_bytes_sent>\\d{1, 3})'}, ignore_partition=True)
