...
class TestGrafanaClient:
    ...
    def init_default_parameters(self, httpserver: HTTPServer):
        ...
        self.athena_connection = AthenaConnection(database='centralized',
                                             account_id='123456789012', 
                                             region='us-east-1', 
                                             work_group='Primary', 
                                             assume_role_arn='arn:aws:iam::123456789012:role/AthenaPublicAccessRole',
        ...
        httpserver.expect_request(uri='/api/datasources/name/Athena-clo-123456789012-us-east-1', method='GET').respond_with_data(response_data=json.dumps({'id': 96, 'name': 'Athena-clo-123456789012-us-east-1', 'uid': 'g5Aeh9_Vk', 'type': 'grafana-athena-datasource'}))
        httpserver.expect_request(uri='/api/datasources/uid/g5Aeh9_Vk', method='PUT').respond_with_data(response_data=json.dumps({'datasource': {'id': 96, 'uid': 'g5Aeh9_Vk', 'name': 'Athena-clo-123456789012-us-east-1', 'type': 'grafana-athena-datasource'}, 'id': 96, 'message': 'Datasource updated', 'name': 'Athena-clo-123456789012-us-east-1'}))
        httpserver.expect_request(uri='/api/datasources/name/Athena-clo-123456789012-us-east-1-test', method='GET').respond_with_data(response_data=json.dumps({'message': 'Data source not found'}), status=404)
        ...
        httpserver.expect_request(uri='/api/folders', method='GET').respond_with_data(response_data=json.dumps([]))
        ...
        self.grafana_client = GrafanaClient(url=self.grafana_url, token=self.grafana_token, verify=False)
    ...
    def test_check_token_validity(self, httpserver: HTTPServer):
        ...
        httpserver.expect_oneshot_request(uri='/api/user/preferences', method='GET').respond_with_data(status=401)
    ...
    def test_has_installed_athena_plugin(self, httpserver: HTTPServer):
        ...
        httpserver.clear()
        httpserver.expect_request(uri='/api/plugins/grafana-athena-datasource/settings', method='GET').respond_with_data(response_data=json.dumps({'message': 'Plugin not found, no installed plugin with that id'}), status=404)
    ...
    def test_check_data_source_permission(self, httpserver: HTTPServer):
        ...
        httpserver.expect_request(uri='/api/datasources/name/TestCreateAthenaDatasource', method='GET').respond_with_data(response_data=json.dumps({'message': 'Data source not found'}), status=404)
        httpserver.expect_request(uri='/api/datasources', method='POST').respond_with_data(status=403)
    ...
    def test_check_folder_permission(self, httpserver: HTTPServer):
        ...
        httpserver.expect_oneshot_request(uri='/api/folders', method='GET').respond_with_data(status=401)
        httpserver.expect_oneshot_request(uri='/api/folders', method='POST').respond_with_data(status=401)
    ...
    def test_check_permission(self, httpserver: HTTPServer):
        ...
        assert exception_info.value.args[0] == 'Failed to check url connectivity, grafana address: http://localhost cannot be accessed.'
        self.grafana_client.url = httpserver.url_for('')
        
        httpserver.expect_oneshot_request(uri='/api/user/preferences', method='GET').respond_with_data(status=401)
        with pytest.raises(Exception) as exception_info:
            self.grafana_client.check_permission()
        assert exception_info.value.args[0] == f'Invalid service account token: {self.grafana_token}'
        httpserver.expect_request(uri='/api/user/preferences', method='GET').respond_with_data(response_data=json.dumps({}))
        
        httpserver.expect_oneshot_request(uri='/api/plugins/grafana-athena-datasource/settings', method='GET').respond_with_data(response_data=json.dumps({'message': 'Plugin not found, no installed plugin with that id'}), status=404)
        with pytest.raises(Exception) as exception_info:
            self.grafana_client.check_permission()
        ...
        httpserver.expect_oneshot_request(uri='/api/datasources/name/TestCreateAthenaDatasource', method='GET').respond_with_data(response_data=json.dumps({'message': 'Data source not found'}), status=404)
        httpserver.expect_oneshot_request(uri='/api/datasources', method='POST').respond_with_data(status=403)
        with pytest.raises(Exception) as exception_info:
            self.grafana_client.check_permission()
        assert exception_info.value.args[0] == 'Missing data source creation permission.'
        httpserver.expect_request(uri='/api/datasources/name/TestCreateAthenaDatasource', method='GET').respond_with_data(response_data=json.dumps({'message': 'Data source not found'}), status=404)
        httpserver.expect_request(uri='/api/datasources', method='POST').respond_with_data(response_data=json.dumps({'datasource': {'id': 98, 'uid': 'BasXA4g_ar', 'name': 'Athena', 'type': 'grafana-athena-datasource'}, 'id': 98, 'message': 'Datasource added', 'name': 'TestCreateAthenaDatasource'}))
        httpserver.expect_request(uri='/api/datasources/uid/BasXA4g_ar', method='DELETE').respond_with_data(status=200)
    ...
    def test_get_folder_by_title(self, httpserver: HTTPServer):
        ...
        response = self.grafana_client.get_folder_by_title(title='clo')
        assert response == {'status': 200, 'content': {'id': 19, 'uid': 'zypaSkX4k', 'title': 'clo'}}
    ...
    def test_create_folder(self, httpserver: HTTPServer):
        ...
        assert response == {'status': 200, 'content': {'id': 19, 'uid': 'zypaSkX4k', 'title': 'clo', 'url': '/dashboards/f/zypaSkX4k/clo', 'hasAcl': False, 'canSave': True, 'canEdit': True, 'canAdmin': True, 'canDelete': True, 'createdBy': 'Anonymous', 'updatedBy': 'Anonymous','version': 1}}
        ...
        response = self.grafana_client.create_folder(title='clo')
        assert response == {'status': 200, 'content': {'id': 19, 'uid': 'zypaSkX4k', 'title': 'clo', 'url': '/dashboards/f/zypaSkX4k/clo', 'hasAcl': False, 'canSave': True, 'canEdit': True, 'canAdmin': True, 'canDelete': True, 'version': 1}}
    ...
    def test_update_datasource_by_name(self, httpserver: HTTPServer):
        ...
        response = self.grafana_client.update_datasource_by_name(datasource_name='Athena-clo-123456789012-us-east-1', datasource=datasource)
        assert response['status'] == 200
        assert response['content']['message'] == 'Datasource updated'
        
        httpserver.expect_request(uri='/api/datasources', method='POST').respond_with_data(response_data=json.dumps({'datasource': {'id': 97, 'uid': 'AxpRJ3l_4k', 'name': 'Athena-clo-123456789012-us-east-1-test', 'type': 'grafana-athena-datasource'}, 'id': 97, 'message': 'Datasource added', 'name': 'Athena-clo-123456789012-us-east-1-test'}))

        response = self.grafana_client.update_datasource_by_name(datasource_name='Athena-clo-123456789012-us-east-1-test', datasource=datasource)
        assert response['status'] == 200
        assert response['content']['message'] == 'Datasource added'
        
        httpserver.clear()
        httpserver.expect_request(uri='/api/datasources/name/Athena-clo-123456789012-us-east-1-test', method='GET').respond_with_data(response_data=json.dumps({'message': 'Data source not found'}), status=404)
        httpserver.expect_request(uri='/api/datasources', method='POST').respond_with_data(status=400)
...
class TestGrafanaApi:
    ...
    def init_default_parameters(self, httpserver: HTTPServer):
        ...
        httpserver.expect_request(uri='/api/datasources/uid/g5Aeh9_Vk', method='PUT').respond_with_data(response_data=json.dumps({'datasource': {'id': 96, 'uid': 'g5Aeh9_Vk', 'name': 'Athena-clo-123456789012-us-east-1', 'type': 'grafana-athena-datasource'}, 'id': 96, 'message': 'Datasource updated', 'name': 'Athena-clo-123456789012-us-east-1'}))
        httpserver.expect_request(uri='/api/dashboards/uid/w0x1c_Y4k5', method='DELETE').respond_with_data(response_data=json.dumps({'id': 7, 'message': 'Dashboard deleted', 'title': 'Dashboard'}))
        
        self.grafana_api = GrafanaApi(url=self.grafana_url, token=self.grafana_token, verify=False)
    ...
    def test_init(self, httpserver: HTTPServer):
        ...
        assert grafana_api.api.headers['Authorization'] == f"Bearer {self.grafana_token}"
        
        grafana_api= GrafanaApi(url=self.grafana_url, token=self.grafana_token, verify=True, timeout=10)
        assert grafana_api.url == f'{self.grafana_url}/api'
        assert grafana_api.token == self.grafana_token
    ...
    def test_get(self, httpserver: HTTPServer):
        ...
        response = self.grafana_api.get('/datasources/name/Athena-clo-123456789012-us-east-1')
        assert response == {'status': 200, 'content': {'id': 96, 'name': 'Athena-clo-123456789012-us-east-1', 'uid': 'g5Aeh9_Vk', 'type': 'grafana-athena-datasource'}}
    ...
    def test_put(self, httpserver: HTTPServer):
        ...
        assert response == {'status': 200, 'content': {'datasource': {'id': 96, 'uid': 'g5Aeh9_Vk', 'name': 'Athena-clo-123456789012-us-east-1', 'type': 'grafana-athena-datasource'}, 'id': 96, 'message': 'Datasource updated', 'name': 'Athena-clo-123456789012-us-east-1'}}
    
    def test_delete(self, httpserver: HTTPServer):
        self.init_default_parameters(httpserver=httpserver)
        
        response = self.grafana_api.delete('/dashboards/uid/w0x1c_Y4k5')
        assert response == {'status': 200, 'content': {'id': 7, 'message': 'Dashboard deleted', 'title': 'Dashboard'}}
    
    def test_request(self, httpserver: HTTPServer):
        self.init_default_parameters(httpserver=httpserver)
        
        response = self.grafana_api._request(path='/datasources/name/Athena-clo-123456789012-us-east-1', method='GET')
        assert response == {'status': 200, 'content': {'id': 96, 'name': 'Athena-clo-123456789012-us-east-1', 'uid': 'g5Aeh9_Vk', 'type': 'grafana-athena-datasource'}}

        response = self.grafana_api._request(path='/dashboards/db', method='POST', json=None)
        assert response == {'status': 200, 'content': {'id': 7, 'status': 'success', 'uid': 'w0x1c_Y4k5', 'version': 1}}
    
        response = self.grafana_api._request(path='/datasources/uid/g5Aeh9_Vk', method='PUT', json=None)
        assert response == {'status': 200, 'content': {'datasource': {'id': 96, 'uid': 'g5Aeh9_Vk', 'name': 'Athena-clo-123456789012-us-east-1', 'type': 'grafana-athena-datasource'}, 'id': 96, 'message': 'Datasource updated', 'name': 'Athena-clo-123456789012-us-east-1'}}
    
        response = self.grafana_api._request(path='/dashboards/uid/w0x1c_Y4k5', method='DELETE', json=None)
        assert response == {'status': 200, 'content': {'id': 7, 'message': 'Dashboard deleted', 'title': 'Dashboard'}}
        
        httpserver.expect_oneshot_request(uri='/invalid/request', method='GET').respond_with_data(response_data='')
        response = self.grafana_api._request(path='/invalid/request', method='GET', json=None)
        assert response == {'status': 400}
        
        httpserver.expect_oneshot_request(uri='/invalid/request', method='GET').respond_with_data(response_data='<html></html>')
...
class TestPlugin:
    ...
    def test_get_plugin_settings(self, httpserver: HTTPServer):
        ...
        assert response == {'status': 200, 'content': {'name': 'Amazon Athena', 'type': 'datasource', 'id': 'grafana-athena-datasource', 'enabled': False, 'pinned': False}}
...
class TestDataSource:
    ...
    def init_default_parameters(self, httpserver: HTTPServer):
        ...
        httpserver.expect_request(uri='/api/datasources/name/Athena-clo-123456789012-us-east-1', method='GET').respond_with_data(response_data=json.dumps({'id': 96, 'name': 'Athena-clo-123456789012-us-east-1', 'uid': 'g5Aeh9_Vk', 'type': 'grafana-athena-datasource'}))
        httpserver.expect_request(uri='/api/datasources/uid/g5Aeh9_Vk', method='PUT').respond_with_data(response_data=json.dumps({'datasource': {'id': 96, 'uid': 'g5Aeh9_Vk', 'name': 'Athena-clo-123456789012-us-east-1', 'type': 'grafana-athena-datasource'}, 'id': 96, 'message': 'Datasource updated', 'name': 'Athena-clo-123456789012-us-east-1'}))
        httpserver.expect_request(uri='/api/datasources/uid/g5Aeh9_Vk', method='GET').respond_with_data(response_data=json.dumps({'id': 98, 'uid': 'g5Aeh9_Vk', 'name': 'Athena-clo-123456789012-us-east-1', 'type': 'grafana-athena-datasource'}))
        httpserver.expect_request(uri='/api/datasources/uid/g5Aeh9_Vk', method='DELETE').respond_with_data(response_data=json.dumps({'id': 98, 'message': 'Data source deleted'}))
    ...
    def test_get_datasource(self, httpserver: HTTPServer):
        ...
        assert response == {'status': 200, 'content': {'id': 98, 'uid': 'g5Aeh9_Vk', 'name': 'Athena-clo-123456789012-us-east-1', 'type': 'grafana-athena-datasource'}}
    
    def test_get_datasource_by_name(self, httpserver: HTTPServer):
        self.init_default_parameters(httpserver=httpserver)

        response = self.datasource_api.get_datasource_by_name(datasource_name='Athena-clo-123456789012-us-east-1')
        assert response == {'status': 200, 'content': {'id': 96, 'name': 'Athena-clo-123456789012-us-east-1', 'uid': 'g5Aeh9_Vk', 'type': 'grafana-athena-datasource'}}
    
    def test_create_datasource(self, httpserver: HTTPServer):
        self.init_default_parameters(httpserver=httpserver)
        
        httpserver.expect_oneshot_request(uri='/api/datasources', method='POST').respond_with_data(response_data=json.dumps({'datasource': {'id': 101, 'uid': 'l5y8izu4z', 'name': 'Athena-clo-942636716027-us-east-1', 'type': 'grafana-athena-datasource'}}))
        datasource = {
            "type": "grafana-athena-datasource",
            "access": "proxy",
            "isDefault": False,
        ...
        assert response == {'status': 200, 'content': {'datasource': {'id': 101, 'uid': 'l5y8izu4z', 'name': 'Athena-clo-942636716027-us-east-1', 'type': 'grafana-athena-datasource'}}}
    ...
    def test_update_datasource(self, httpserver: HTTPServer):
        ...
        response = self.datasource_api.update_datasource(datasource_uid='g5Aeh9_Vk', datasource=datasource)
        assert response == {'status': 200, 'content': {'datasource': {'id': 96, 'uid': 'g5Aeh9_Vk', 'name': 'Athena-clo-123456789012-us-east-1', 'type': 'grafana-athena-datasource'}, 'id': 96, 'message': 'Datasource updated', 'name': 'Athena-clo-123456789012-us-east-1'}}
    ...
    def test_delete_datasource(self, httpserver: HTTPServer):
        ...
        assert response == {'status': 200, 'content': {'id': 98, 'message': 'Data source deleted'}}
...
class TestDashboard:
    ...
    def init_default_parameters(self, httpserver: HTTPServer):
        ...
        httpserver.expect_request(uri='/api/dashboards/uid/0HklGl_Vz', method='GET').respond_with_data(response_data=json.dumps({'meta': {'type': 'db', 'canSave': True, 'canEdit': True, 'canAdmin': True, 'canStar': True, 'canDelete': True, 'slug': 'production-overview', 'url': '/d/0HklGl_Vz/production-overview', 'expires': '0001-01-01T00:00:00Z', 'created': '2023-06-12T08:46:20Z', 'updated': '2023-06-12T08:46:20Z', 'updatedBy': 'Anonymous', 'createdBy': 'Anonymous', 'version': 1, 'hasAcl': False, 'isFolder': False, 'folderId': 0, 'folderUid': '', 'folderTitle': 'General', 'folderUrl': '', 'provisioned': False, 'provisionedExternalId': '', 'annotationsPermissions': {'dashboard': {'canAdd': True, 'canEdit': True, 'canDelete': True}, 'organization': {'canAdd': True, 'canEdit': True, 'canDelete': True}}, 'publicDashboardAccessToken': '', 'publicDashboardUid': '', 'publicDashboardEnabled': False}, 'dashboard': {'id': 14, 'refresh': '25s', 'schemaVersion': 16, 'tags': ['templated'], 'timezone': 'browser', 'title': 'Production Overview', 'uid': '0HklGl_Vz', 'version': 1}}))
        httpserver.expect_request(uri='/api/dashboards/db', method='POST').respond_with_data(response_data=json.dumps({'id': 17, 'slug': 'production-overview', 'status': 'success', 'uid': '4b122cbc-1d26-4e63-b05a-0d2fdd69c2ff-00', 'url': '/d/4b122cbc-1d26-4e63-b05a-0d2fdd69c2ff-00/production-overview', 'version': 58}))
        httpserver.expect_request(uri='/api/dashboards/uid/0HklGl_Vz', method='DELETE').respond_with_data(response_data=json.dumps({'id': 17, 'message': 'Dashboard Production Overview deleted', 'title': 'Production Overview'}))
    ...
    def test_get_dashboard(self, httpserver: HTTPServer):
        ...
        assert response == {'status': 200, 'content': {'meta': {'type': 'db', 'canSave': True, 'canEdit': True, 'canAdmin': True, 'canStar': True, 'canDelete': True, 'slug': 'production-overview', 'url': '/d/0HklGl_Vz/production-overview', 'expires': '0001-01-01T00:00:00Z', 'created': '2023-06-12T08:46:20Z', 'updated': '2023-06-12T08:46:20Z', 'updatedBy': 'Anonymous', 'createdBy': 'Anonymous', 'version': 1, 'hasAcl': False, 'isFolder': False, 'folderId': 0, 'folderUid': '', 'folderTitle': 'General', 'folderUrl': '', 'provisioned': False, 'provisionedExternalId': '', 'annotationsPermissions': {'dashboard': {'canAdd': True, 'canEdit': True, 'canDelete': True}, 'organization': {'canAdd': True, 'canEdit': True, 'canDelete': True}}, 'publicDashboardAccessToken': '', 'publicDashboardUid': '', 'publicDashboardEnabled': False}, 'dashboard': {'id': 14, 'refresh': '25s', 'schemaVersion': 16, 'tags': ['templated'], 'timezone': 'browser', 'title': 'Production Overview', 'uid': '0HklGl_Vz', 'version': 1}}}
    ...
    def test_update_dashboard(self, httpserver: HTTPServer):
        ...
        assert response == {'status': 200, 'content': {'id': 17, 'slug': 'production-overview', 'status': 'success', 'uid': '4b122cbc-1d26-4e63-b05a-0d2fdd69c2ff-00', 'url': '/d/4b122cbc-1d26-4e63-b05a-0d2fdd69c2ff-00/production-overview', 'version': 58}}
    
    def test_delete_dashboard(self, httpserver: HTTPServer):
        self.init_default_parameters(httpserver=httpserver)
        
        response = self.dashboard_api.delete_dashboard(dashboard_uid='0HklGl_Vz')
        assert response == {'status': 200, 'content': {'id': 17, 'message': 'Dashboard Production Overview deleted', 'title': 'Production Overview'}}
...
class TestFolder:
    ...
    def init_default_parameters(self, httpserver: HTTPServer):
        ...
        httpserver.expect_request(uri='/api/folders/zypaSkX4k', method='GET').respond_with_data(response_data=json.dumps({'id': 19, 'uid': 'zypaSkX4k', 'title': 'clo', 'url': '/dashboards/f/zypaSkX4k/clo', 'hasAcl': False, 'canSave': True, 'canEdit': True, 'canAdmin': True, 'canDelete': True, 'version': 1}))
        httpserver.expect_request(uri='/api/folders/0HklGl_Vz', method='DELETE').respond_with_data(response_data='')
    ...
    def test_list_folder(self, httpserver: HTTPServer):
        ...
        assert response == {'status': 200, 'content': [{'id': 1, 'uid': 'q_XODzX4z', 'title': 'General'}, {'id': 19, 'uid': 'zypaSkX4k', 'title': 'clo'}]}
    ...
    def test_create_folder(self, httpserver: HTTPServer):
        ...
        httpserver.expect_oneshot_request(uri='/api/folders', method='POST').respond_with_data(response_data=json.dumps({'id': 19, 'uid': 'clo', 'title': 'clo', 'url': '/dashboards/f/clo/clo', 'hasAcl': False, 'canSave': True, 'canEdit': True, 'canAdmin': True, 'canDelete': True, 'createdBy': 'Anonymous', 'updatedBy': 'Anonymous','version': 1}))
        
        response = self.folder_api.create_folder(title='clo', folder_uid='clo')
        assert response == {'status': 200, 'content': {'id': 19, 'uid': 'clo', 'title': 'clo', 'url': '/dashboards/f/clo/clo', 'hasAcl': False, 'canSave': True, 'canEdit': True, 'canAdmin': True, 'canDelete': True, 'createdBy': 'Anonymous', 'updatedBy': 'Anonymous','version': 1}}
...
class TestUser:
    ...
    def test_preferences(self, httpserver: HTTPServer):
        ...
        httpserver.expect_oneshot_request(uri='/api/user/preferences', method='GET').respond_with_data(response_data=json.dumps({}), status=200)

        response = self.user_api.preferences()
        assert response == {'status': 200, 'content': {}}
        
        httpserver.expect_oneshot_request(uri='/api/user/preferences', method='GET').respond_with_data(response_data=json.dumps({}), status=401)
