...
from test.mock import mock_sts_context, mock_iam_context, mock_sqs_context, mock_ddb_context, mock_s3_context, default_environment_variables
...
def test_file_reader():
    ...
    assert isinstance(file_reader(path=f'{current_path}/data/apigateway1.log'), io.TextIOWrapper) is True
    assert isinstance(file_reader(path=f'{current_path}/data/apigateway1.log', extension='text'), io.TextIOWrapper) is True
    assert isinstance(file_reader(path=f'{current_path}/data/alb.log.gz', extension='gz'), io.TextIOWrapper) is True
...
def test_file_writer():
    ...
    assert isinstance(file_writer(path=f'{tmp_path}/alb1.log'), io.TextIOWrapper) is True
    assert isinstance(file_writer(path=f'{tmp_path}/alb2.log', extension='text'), io.TextIOWrapper) is True
    assert isinstance(file_writer(path=f'{tmp_path}/alb.log.gz', extension='gz'), io.TextIOWrapper) is True
...
def test_detect_file_extension_by_header():
    ...
    assert detect_file_extension_by_header(current_dir / 'data/apigateway1.log') == 'text'
    assert detect_file_extension_by_header(current_dir / 'data/apigateway1.gz') == 'gz'
    assert detect_file_extension_by_header(current_dir / 'data/apigateway1.parquet') == 'parquet'
...
def test_enrichment():
        ...
        assert enrichment(input_file_path=Path(input_filename), output_file_path=Path(output_filename), enrich_func=enrich_func, enrich_plugins=set()).as_posix() == input_filename
        ...
        enrichment(input_file_path=Path(input_filename), output_file_path=Path(output_filename), enrich_func=enrich_func, enrich_plugins=set())
        ...
        enrichment(input_file_path=Path(text_input_filename), output_file_path=Path(text_output_filename), enrich_func=enrich_func)
