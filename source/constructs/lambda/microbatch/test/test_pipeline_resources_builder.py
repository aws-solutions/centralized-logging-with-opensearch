...
class TestParameter:
    def test_required_parameter_check(
        ...
        from pipeline_resources_builder.lambda_function import (
            Parameters,
            MetaTable,
            dataformat,
        )
        ...
        create_waf_pipeline_event = {
            "RequestType": "Create",
            "ResourceProperties": {"Resource": "pipeline", "Id": waf_pipeline_id},
        }
        create_waf_ingestion_event = {
            "RequestType": "Create",
            "ResourceProperties": {"Resource": "ingestion", "Id": waf_ingestion_id},
        }
        ...
        assert (
            ...
            == f'Not supported Resource: {event["ResourceProperties"]["Resource"]}, supported: pipeline, ingestion'
        )
    ...
    def test_staging_bucket(
        ...
        assert (
            param.staging_bucket == AWS_DDB_META.get(meta_name="StagingBucket")["name"]
        )

    def test_scheduler_is_available(
        self, mock_s3_context, mock_iam_context, mock_sqs_context, mock_ddb_context
    ):
        from pipeline_resources_builder.lambda_function import Parameters, MetaTable
    ...
    def test_get_scheduler_info(
        ...
        log_processor_execution_role_arn = AWS_DDB_META.get(
            meta_name="LogProcessorStartExecutionRole"
        )["arn"]
        log_merger_arn = AWS_DDB_META.get(meta_name="LogMerger")["arn"]
        log_merger_execution_role_arn = AWS_DDB_META.get(
            meta_name="LogMergerStartExecutionRole"
        )["arn"]
        log_archive_arn = AWS_DDB_META.get(meta_name="LogArchive")["arn"]
        log_archive_execution_role_arn = AWS_DDB_META.get(
            meta_name="LogArchiveStartExecutionRole"
        )["arn"]
        waf_pipeline_info = AWS_DDB_META.get(meta_name=waf_pipeline_id)
        ...
        log_processor = param._get_scheduler_info(
            schedule=waf_pipeline_info["data"]["scheduler"]["LogProcessor"]["schedule"],
        ...
        assert (
            log_processor.schedule
            == waf_pipeline_info["data"]["scheduler"]["LogProcessor"]["schedule"]
        )
        assert log_processor.age == waf_pipeline_info["data"]["scheduler"][
            "LogProcessor"
        ].get("age", -1)

        log_merger = param._get_scheduler_info(
            schedule=waf_pipeline_info["data"]["scheduler"]["LogMerger"]["schedule"],
            age=waf_pipeline_info["data"]["scheduler"]["LogMerger"].get("age"),
        ...
        assert (
            log_merger.schedule
            == waf_pipeline_info["data"]["scheduler"]["LogMerger"]["schedule"]
        )
        assert log_merger.age == waf_pipeline_info["data"]["scheduler"][
            "LogMerger"
        ].get("age")

        log_archive = param._get_scheduler_info(
            schedule=waf_pipeline_info["data"]["scheduler"]["LogArchive"]["schedule"],
            age=waf_pipeline_info["data"]["scheduler"]["LogArchive"].get("age"),
        ...
        assert (
            log_archive.schedule
            == waf_pipeline_info["data"]["scheduler"]["LogArchive"]["schedule"]
        )
        assert log_archive.age == waf_pipeline_info["data"]["scheduler"][
            "LogArchive"
        ].get("age")
    ...
    def test_get_bucket_object(
        ...
        bucket_object = param._get_bucket_object(
            bucket=centralized_bucket_name, prefix="///AWS/WAFLogs//"
        )
        ...
        assert (
            bucket_object.archive_uri
            == f"s3://{staging_bucket_name}/archive/AWS/WAFLogs"
        )

        bucket_object = param._get_bucket_object(
            bucket=centralized_bucket_name, prefix="/"
        )
    ...
    def test_get_staging_bucket_info(
        self, mock_s3_context, mock_iam_context, mock_sqs_context, mock_ddb_context
    ):
        from pipeline_resources_builder.lambda_function import Parameters, MetaTable
        ...
        assert (
            staging_bucket_object.archive_uri == f"s3://{staging_bucket_name}/archive/"
        )
        ...
        staging_bucket_object = param._get_staging_bucket_info(
            prefix=waf_pipeline_info["data"]["staging"]["prefix"]
        )

        assert staging_bucket_object.bucket == staging_bucket_name
        assert (
            staging_bucket_object.prefix
            == waf_pipeline_info["data"]["staging"]["prefix"]
        )
        assert staging_bucket_object.arn == f"arn:aws:s3:::{staging_bucket_name}"
        assert (
            staging_bucket_object.uri
            == f"s3://{staging_bucket_name}/{waf_pipeline_info['data']['staging']['prefix']}"
        )
        assert (
            staging_bucket_object.archive_uri
            == f"s3://{staging_bucket_name}/archive/{waf_pipeline_info['data']['staging']['prefix']}"
        )

    def test_get_destination_info(
        self, mock_s3_context, mock_iam_context, mock_sqs_context, mock_ddb_context
    ):
        from pipeline_resources_builder.lambda_function import Parameters, MetaTable
        ...
        destination = param._get_destination_info(
            info=waf_pipeline_info["data"]["destination"]
        )

        assert (
            destination.location.prefix
            == waf_pipeline_info["data"]["destination"]["location"]["prefix"]
        )
        assert (
            destination.location.arn
            == f"arn:aws:s3:::{waf_pipeline_info['data']['destination']['location']['bucket']}"
        )
        assert (
            destination.location.uri
            == f"s3://{waf_pipeline_info['data']['destination']['location']['bucket']}/{waf_pipeline_info['data']['destination']['location']['prefix']}"
        )
        assert (
            destination.location.archive_uri
            == f"s3://{staging_bucket_name}/archive/{waf_pipeline_info['data']['destination']['location']['prefix']}"
        )
        assert (
            destination.database.name
            == waf_pipeline_info["data"]["destination"]["database"]["name"]
        )
        assert (
            destination.database.location.bucket
            == waf_pipeline_info["data"]["destination"]["location"]["bucket"]
        )
        assert (
            destination.database.location.prefix
            == f"{waf_pipeline_info['data']['destination']['location']['prefix']}/{waf_pipeline_info['data']['destination']['database']['name']}"
        )
        assert (
            destination.database.location.arn
            == f"arn:aws:s3:::{waf_pipeline_info['data']['destination']['location']['bucket']}"
        )
        assert (
            destination.database.location.uri
            == f"s3://{waf_pipeline_info['data']['destination']['location']['bucket']}/{waf_pipeline_info['data']['destination']['location']['prefix']}/{waf_pipeline_info['data']['destination']['database']['name']}"
        )
        assert (
            destination.database.location.archive_uri
            == f"s3://{staging_bucket_name}/archive/{waf_pipeline_info['data']['destination']['location']['prefix']}/{waf_pipeline_info['data']['destination']['database']['name']}"
        )
        assert (
            destination.table.name
            == waf_pipeline_info["data"]["destination"]["table"]["name"]
        )
        assert destination.table.schema == json.loads(
            waf_pipeline_info["data"]["destination"]["table"].get("schema", "{}")
        )
        assert (
            destination.table.location.bucket
            == waf_pipeline_info["data"]["destination"]["location"]["bucket"]
        )
        assert (
            destination.table.location.prefix
            == f"{waf_pipeline_info['data']['destination']['location']['prefix']}/{waf_pipeline_info['data']['destination']['database']['name']}/{waf_pipeline_info['data']['destination']['table']['name']}"
        )
        assert (
            destination.table.location.arn
            == f"arn:aws:s3:::{waf_pipeline_info['data']['destination']['location']['bucket']}"
        )
        assert (
            destination.table.location.uri
            == f"s3://{waf_pipeline_info['data']['destination']['location']['bucket']}/{waf_pipeline_info['data']['destination']['location']['prefix']}/{waf_pipeline_info['data']['destination']['database']['name']}/{waf_pipeline_info['data']['destination']['table']['name']}"
        )
        assert (
            destination.table.location.archive_uri
            == f"s3://{staging_bucket_name}/archive/{waf_pipeline_info['data']['destination']['location']['prefix']}/{waf_pipeline_info['data']['destination']['database']['name']}/{waf_pipeline_info['data']['destination']['table']['name']}"
        )
        assert (
            destination.metrics.name
            == waf_pipeline_info["data"]["destination"]["metrics"]["name"]
            if waf_pipeline_info["data"]["destination"]["metrics"]["name"]
            else f"{waf_pipeline_info['data']['destination']['table']['name']}_metrics"
        )
        assert destination.metrics.schema == json.loads(
            waf_pipeline_info["data"]["destination"]["metrics"].get("schema", "{}")
        )
        assert (
            destination.metrics.location.bucket
            == waf_pipeline_info["data"]["destination"]["location"]["bucket"]
        )
        assert (
            destination.metrics.location.prefix
            == f"{waf_pipeline_info['data']['destination']['location']['prefix']}/{waf_pipeline_info['data']['destination']['database']['name']}/{waf_pipeline_info['data']['destination']['table']['name']}_metrics"
        )
        assert (
            destination.metrics.location.arn
            == f"arn:aws:s3:::{waf_pipeline_info['data']['destination']['location']['bucket']}"
        )
        assert (
            destination.metrics.location.uri
            == f"s3://{waf_pipeline_info['data']['destination']['location']['bucket']}/{waf_pipeline_info['data']['destination']['location']['prefix']}/{waf_pipeline_info['data']['destination']['database']['name']}/{waf_pipeline_info['data']['destination']['table']['name']}_metrics"
        )
        assert (
            destination.metrics.location.archive_uri
            == f"s3://{staging_bucket_name}/archive/{waf_pipeline_info['data']['destination']['location']['prefix']}/{waf_pipeline_info['data']['destination']['database']['name']}/{waf_pipeline_info['data']['destination']['table']['name']}_metrics"
        )
        ...
        waf_pipeline_info_copy["data"]["destination"]["table"]["schema"] = json.dumps(
            table_schema
        )
        waf_pipeline_info_copy["data"]["destination"]["metrics"][
            "name"
        ] = "aws_waf_logs_metrics"
        waf_pipeline_info_copy["data"]["destination"]["metrics"]["schema"] = json.dumps(
            metrics_schema
        )
        ...
        destination = param._get_destination_info(
            info=waf_pipeline_info_copy["data"]["destination"]
        )
        ...
        assert (
            destination.metrics.location.prefix
            == f"{waf_pipeline_info['data']['destination']['location']['prefix']}/{waf_pipeline_info['data']['destination']['database']['name']}/{destination.metrics.name}"
        )
        assert (
            destination.metrics.location.arn
            == f"arn:aws:s3:::{waf_pipeline_info['data']['destination']['location']['bucket']}"
        ...
        assert (
            destination.metrics.location.uri
            == f"s3://{waf_pipeline_info['data']['destination']['location']['bucket']}/{waf_pipeline_info['data']['destination']['location']['prefix']}/{waf_pipeline_info['data']['destination']['database']['name']}/{destination.metrics.name}"
        )
        assert (
            destination.metrics.location.archive_uri
            == f"s3://{staging_bucket_name}/archive/{waf_pipeline_info['data']['destination']['location']['prefix']}/{waf_pipeline_info['data']['destination']['database']['name']}/{destination.metrics.name}"
        )
    ...
    def test_parse_table_schema(
        ...
        assert (
            exception_info.value.args[0]
            == f"{input_string} is not a valid JSON string."
        )
        ...
        assert (
            exception_info.value.args[0]
            == f"{new_input_string} is not a valid JSON string."
        )

    def test_validate_source_table_attribute(
        self, mock_s3_context, mock_iam_context, mock_sqs_context, mock_ddb_context
    ):
        from pipeline_resources_builder.lambda_function import (
            Parameters,
        ...
        for source_type in ("fluent-bit", "s3"):
            table_info = {}
            with pytest.raises(Exception) as exception_info:
                param._validate_source_table_attribute(
                    source_type=source_type, table_info=table_info
                )
            assert (
                exception_info.value.args[0]
                == f"Unsupported data format: , supported value: {list(dataformat.DATA_FORMAT_MAPPING.keys())}."
            )
            ...
            assert (
                exception_info.value.args[0]
                == f"Unsupported data format: {table_info['dataFormat']}, supported value: {list(dataformat.DATA_FORMAT_MAPPING.keys())}."
            )
            ...
            assert (
                exception_info.value.args[0]
                == f'{table_info["tableProperties"]} is not a valid JSON string.'
            )
            ...
            assert (
                exception_info.value.args[0]
                == f'{table_info["serializationProperties"]} is not a valid JSON string.'
            )
    ...
    def test_get_pipeline_source_info(
        self, mock_s3_context, mock_iam_context, mock_sqs_context, mock_ddb_context
    ):
        from pipeline_resources_builder.lambda_function import (
            Parameters,
        ...
        tmp_database_name = AWS_DDB_META.get(meta_name="TmpDatabase")["name"]
        waf_pipeline_info = AWS_DDB_META.get(meta_name=waf_pipeline_id)
        waf_pipeline_info["data"]["source"]["table"][
            "name"
        ] = f"{waf_pipeline_info['data']['destination']['table']['name']}{{}}"
        ...
        with pytest.raises(ValueError):
            param._get_pipeline_source_info(
                info={
                    "type": "do-not-supported",
                    "table": {
                        "name": waf_pipeline_info["data"]["destination"]["table"][
                            "name"
                        ],
                        "schema": "{}",
                        "dataFormat": "",
        ...
        source = param._get_pipeline_source_info(
            info=waf_pipeline_info["data"]["source"]
        )
        ...
        assert source.table.schema == json.loads(
            waf_pipeline_info["data"]["source"]["table"].get("schema", "{}")
        )
        assert source.table.data_format == dataformat.DATA_FORMAT_MAPPING.get(
            waf_pipeline_info["data"]["source"]["table"].get("dataFormat"), ""
        )
        assert source.table.table_properties == json.loads(
            waf_pipeline_info["data"]["source"]["table"].get("tableProperties", "{}")
        )
        assert source.table.serialization_properties == json.loads(
            waf_pipeline_info["data"]["source"]["table"].get(
                "serializationProperties", ""
            )
        )
    ...
    def test_get_pipeline_grafana_info(
        self, mock_s3_context, mock_iam_context, mock_sqs_context, mock_ddb_context
    ):
        from pipeline_resources_builder.lambda_function import Parameters, MetaTable
        ...
        grafana = param._get_pipeline_grafana_info(
            info=waf_pipeline_info["data"]["grafana"]
        )
        ...
        with pytest.raises(ValueError):
            param._get_pipeline_grafana_info(
                info={
    ...
    def test_get_pipeline_notification_info(
        self,
        mock_s3_context,
        mock_iam_context,
        mock_sqs_context,
        ...
        notification = param._get_pipeline_notification_info(
            info={"service": "sns", "recipients": receive_states_failed_topic_arn}
        )
    ...
    def test_pipeline_info(
        ...
        assert (
            param.pipeline_info.role.replicate
            == waf_pipeline_info["stack"]["role"]["replicate"]
        )
        ...
        assert (
            param.pipeline_info.scheduler.log_processor.execution_role
            == log_processor_execution_role_arn
        )
        assert (
            param.pipeline_info.scheduler.log_processor.schedule
            == waf_pipeline_info["data"]["scheduler"]["LogProcessor"]["schedule"]
        )
        assert param.pipeline_info.scheduler.log_processor.age == waf_pipeline_info[
            "data"
        ]["scheduler"]["LogProcessor"].get("age", -1)
        assert param.pipeline_info.scheduler.log_merger.arn == log_merger_arn
        assert (
            param.pipeline_info.scheduler.log_merger.execution_role
            == log_merger_execution_role_arn
        )
        assert (
            param.pipeline_info.scheduler.log_merger.schedule
            == waf_pipeline_info["data"]["scheduler"]["LogMerger"]["schedule"]
        )
        assert param.pipeline_info.scheduler.log_merger.age == waf_pipeline_info[
            "data"
        ]["scheduler"]["LogMerger"].get("age")
        assert param.pipeline_info.scheduler.log_archive.arn == log_archive_arn
        assert (
            param.pipeline_info.scheduler.log_archive.execution_role
            == log_archive_execution_role_arn
        )
        assert (
            param.pipeline_info.scheduler.log_archive.schedule
            == waf_pipeline_info["data"]["scheduler"]["LogArchive"]["schedule"]
        )
        assert param.pipeline_info.scheduler.log_archive.age == waf_pipeline_info[
            "data"
        ]["scheduler"]["LogArchive"].get("age")
        assert param.pipeline_info.staging.bucket == staging_bucket_name
        assert (
            param.pipeline_info.staging.prefix
            == waf_pipeline_info["data"]["staging"]["prefix"]
        )
        assert param.pipeline_info.staging.arn == staging_bucket_arn
        assert (
            param.pipeline_info.staging.uri
            == f"s3://{staging_bucket_name}/{waf_pipeline_info['data']['staging']['prefix']}"
        )
        assert (
            param.pipeline_info.staging.archive_uri
            == f"s3://{staging_bucket_name}/archive/{waf_pipeline_info['data']['staging']['prefix']}"
        )
        assert param.pipeline_info.notification.service == "SES"
        assert param.pipeline_info.notification.recipients == waf_pipeline_info["data"][
            "notification"
        ]["recipients"].split(",")
        assert (
            param.pipeline_info.source.type
            == waf_pipeline_info["data"]["source"]["type"].lower()
        )
        assert param.pipeline_info.source.database.name == tmp_database_name
        assert (
            param.pipeline_info.source.table.name
            == f"{waf_pipeline_info['data']['destination']['table']['name']}{{}}"
        )
        assert param.pipeline_info.source.table.schema == json.loads(
            waf_pipeline_info["data"]["source"]["table"].get("schema", "{}")
        )
        assert (
            param.pipeline_info.source.table.data_format
            == dataformat.DATA_FORMAT_MAPPING.get(
                waf_pipeline_info["data"]["source"]["table"].get("dataFormat"), ""
            )
        )
        assert param.pipeline_info.source.table.table_properties == json.loads(
            waf_pipeline_info["data"]["source"]["table"].get("tableProperties", "{}")
        )
        assert param.pipeline_info.source.table.serialization_properties == json.loads(
            waf_pipeline_info["data"]["source"]["table"].get(
                "serializationProperties", ""
            )
        )
        assert (
            param.pipeline_info.destination.location.bucket
            == waf_pipeline_info["data"]["destination"]["location"]["bucket"]
        )
        assert (
            param.pipeline_info.destination.location.prefix
            == waf_pipeline_info["data"]["destination"]["location"]["prefix"]
        )
        assert (
            param.pipeline_info.destination.location.arn
            == f"arn:aws:s3:::{waf_pipeline_info['data']['destination']['location']['bucket']}"
        )
        assert (
            param.pipeline_info.destination.location.uri
            == f"s3://{waf_pipeline_info['data']['destination']['location']['bucket']}/{waf_pipeline_info['data']['destination']['location']['prefix']}"
        )
        assert (
            param.pipeline_info.destination.location.archive_uri
            == f"s3://{staging_bucket_name}/archive/{waf_pipeline_info['data']['destination']['location']['prefix']}"
        )
        assert (
            param.pipeline_info.destination.database.name
            == waf_pipeline_info["data"]["destination"]["database"]["name"]
        )
        assert (
            param.pipeline_info.destination.database.location.bucket
            == waf_pipeline_info["data"]["destination"]["location"]["bucket"]
        )
        assert (
            param.pipeline_info.destination.database.location.prefix
            == f"{waf_pipeline_info['data']['destination']['location']['prefix']}/{waf_pipeline_info['data']['destination']['database']['name']}"
        )
        assert (
            param.pipeline_info.destination.database.location.arn
            == f"arn:aws:s3:::{waf_pipeline_info['data']['destination']['location']['bucket']}"
        )
        assert (
            param.pipeline_info.destination.database.location.uri
            == f"s3://{waf_pipeline_info['data']['destination']['location']['bucket']}/{waf_pipeline_info['data']['destination']['location']['prefix']}/{waf_pipeline_info['data']['destination']['database']['name']}"
        )
        assert (
            param.pipeline_info.destination.database.location.archive_uri
            == f"s3://{staging_bucket_name}/archive/{waf_pipeline_info['data']['destination']['location']['prefix']}/{waf_pipeline_info['data']['destination']['database']['name']}"
        )
        assert (
            param.pipeline_info.destination.table.name
            == waf_pipeline_info["data"]["destination"]["table"]["name"]
        )
        assert param.pipeline_info.destination.table.schema == json.loads(
            waf_pipeline_info["data"]["destination"]["table"].get("schema", "{}")
        )
        assert (
            param.pipeline_info.destination.table.location.bucket
            == waf_pipeline_info["data"]["destination"]["location"]["bucket"]
        )
        assert (
            param.pipeline_info.destination.table.location.prefix
            == f"{waf_pipeline_info['data']['destination']['location']['prefix']}/{waf_pipeline_info['data']['destination']['database']['name']}/{waf_pipeline_info['data']['destination']['table']['name']}"
        )
        assert (
            param.pipeline_info.destination.table.location.arn
            == f"arn:aws:s3:::{waf_pipeline_info['data']['destination']['location']['bucket']}"
        )
        assert (
            param.pipeline_info.destination.table.location.uri
            == f"s3://{waf_pipeline_info['data']['destination']['location']['bucket']}/{waf_pipeline_info['data']['destination']['location']['prefix']}/{waf_pipeline_info['data']['destination']['database']['name']}/{waf_pipeline_info['data']['destination']['table']['name']}"
        )
        assert (
            param.pipeline_info.destination.table.location.archive_uri
            == f"s3://{staging_bucket_name}/archive/{waf_pipeline_info['data']['destination']['location']['prefix']}/{waf_pipeline_info['data']['destination']['database']['name']}/{waf_pipeline_info['data']['destination']['table']['name']}"
        )
        assert (
            param.pipeline_info.destination.metrics.name
            == waf_pipeline_info["data"]["destination"]["metrics"]["name"]
            if waf_pipeline_info["data"]["destination"]["metrics"]["name"]
            else f"{waf_pipeline_info['data']['destination']['table']['name']}_metrics"
        )
        assert param.pipeline_info.destination.metrics.schema == json.loads(
            waf_pipeline_info["data"]["destination"]["metrics"].get("schema", "{}")
        )
        assert (
            param.pipeline_info.destination.metrics.location.bucket
            == waf_pipeline_info["data"]["destination"]["location"]["bucket"]
        )
        assert (
            param.pipeline_info.destination.metrics.location.prefix
            == f"{waf_pipeline_info['data']['destination']['location']['prefix']}/{waf_pipeline_info['data']['destination']['database']['name']}/{param.pipeline_info.destination.metrics.name}"
        )
        assert (
            param.pipeline_info.destination.metrics.location.arn
            == f"arn:aws:s3:::{waf_pipeline_info['data']['destination']['location']['bucket']}"
        )
        assert (
            param.pipeline_info.destination.metrics.location.uri
            == f"s3://{waf_pipeline_info['data']['destination']['location']['bucket']}/{waf_pipeline_info['data']['destination']['location']['prefix']}/{waf_pipeline_info['data']['destination']['database']['name']}/{param.pipeline_info.destination.metrics.name}"
        )
        assert (
            param.pipeline_info.destination.metrics.location.archive_uri
            == f"s3://{staging_bucket_name}/archive/{waf_pipeline_info['data']['destination']['location']['prefix']}/{waf_pipeline_info['data']['destination']['database']['name']}/{param.pipeline_info.destination.metrics.name}"
        )
        ...
        assert (
            param.pipeline_info.grafana.datasource.output_location
            == athena_output_location
        )
        assert (
            param.pipeline_info.grafana.datasource.assume_role_arn
            == athena_public_access_role
        )
        assert (
            param.pipeline_info.queue.log_event_queue
            == waf_pipeline_info["stack"]["queue"]["logEventQueue"]
        )
        assert (
            param.pipeline_info.queue.log_event_dlq
            == waf_pipeline_info["stack"]["queue"]["logEventDLQ"]
        )
        waf_pipeline_info_copy = copy.deepcopy(waf_pipeline_info)
        waf_pipeline_info_copy["data"]["source"]["table"][
            "name"
        ] = f"{waf_pipeline_info['data']['destination']['table']['name']}{{}}"
    ...
    def test_ingestion_info(
        ...
        assert (
            param.ingestion_info.source.location.bucket
            == waf_ingestion_info["data"]["source"]["bucket"]
        )
        assert param.ingestion_info.source.location.prefix == os.path.normpath(
            waf_ingestion_info["data"]["source"]["prefix"]
        )
        assert (
            param.ingestion_info.source.location.arn
            == f"arn:aws:s3:::{waf_ingestion_info['data']['source']['bucket']}"
        )
        assert (
            param.ingestion_info.source.location.uri
            == f"s3://{waf_ingestion_info['data']['source']['bucket']}/{os.path.normpath(waf_ingestion_info['data']['source']['prefix'])}"
        )
        assert (
            param.ingestion_info.source.location.archive_uri
            == f"s3://{staging_bucket_name}/archive/{os.path.normpath(waf_ingestion_info['data']['source']['prefix'])}"
        )
        assert param.ingestion_info.services.s3_event_driver == "SQS"
        assert (
            param.ingestion_info.role.sts == waf_ingestion_info["data"]["role"]["sts"]
        )
        ...
        ingestion_info["data"]["services"].pop("s3EventDriver")
        AWS_DDB_META.put(meta_name=ingestion_id, item=ingestion_info)

        create_ingestion_event = {
            "RequestType": "Create",
            "ResourceProperties": {"Resource": "ingestion", "Id": ingestion_id},
        }
        ...
        assert (
            param.ingestion_info.source.location.bucket
            == ingestion_info["data"]["source"]["bucket"]
        )
        assert param.ingestion_info.source.location.prefix == ""
        assert (
            param.ingestion_info.source.location.arn
            == f"arn:aws:s3:::{ingestion_info['data']['source']['bucket']}"
        )
        assert (
            param.ingestion_info.source.location.uri
            == f"s3://{ingestion_info['data']['source']['bucket']}/"
        )
        assert (
            param.ingestion_info.source.location.archive_uri
            == f"s3://{staging_bucket_name}/archive/"
        )
        assert param.ingestion_info.services.s3_event_driver == "SQS"
        ...
        ingestion_info["data"]["services"]["s3EventDriver"] = ""
        ...
        ingestion_id = str(uuid.uuid4())
        ingestion_info = copy.deepcopy(waf_ingestion_info)
        ingestion_info["data"]["services"]["s3EventDriver"] = "EventBridge"
        AWS_DDB_META.put(meta_name=ingestion_id, item=ingestion_info)
        ...
        assert param.ingestion_info.services.s3_event_driver == "EventBridge"


class TestTableMetaDataGenerator:
    def test_add_fluent_bit_agent_info(
        self,
    ):
        ...
        schema["properties"]["host"] = {"type": "string"}
        new_schema = table_metadata_generator.add_fluent_bit_agent_info(
            schema=schema, table_type="tmp"
        )
        assert schema == {
            "type": "object",
            "properties": collections.OrderedDict(
        ...
        schema["properties"]["host"] = {"type": "string"}
        new_schema = table_metadata_generator.add_fluent_bit_agent_info(
            schema=schema, table_type="centralized"
        )
        assert schema == {
            "type": "object",
            "properties": collections.OrderedDict(
    ...
    def test_find_time_key(
        ...
        time_key = table_metadata_generator.find_time_key(
            schema=schema, source_type="fluent-bit"
        )
        assert time_key == {
            "time": {
                "type": "timestamp",
    ...
    def test_add_time_key(
        ...
        new_schema = table_metadata_generator.add_time_key(
            schema=schema, source_type="fluent-bit"
        )
        assert schema == {
            "type": "object",
            ...
            "properties": collections.OrderedDict(
                [
                    (
                        ...
                        {
                            "type": "big_int",
                            "expression": "CAST(to_unixtime(parse_datetime(\"timestamp\", 'YYYY-MM-dd''T''HH:mm:ss.SSSSSS')) * 1000 AS bigint)",
                        },
                    ),
                    (
                        "timestamp",
                        {
                            "type": "timestamp",
                            "expression": "parse_datetime(\"timestamp\", 'YYYY-MM-dd''T''HH:mm:ss.SSSSSS')",
                        },
                    ),
                    ("host", {"type": "string", "path": '"host"'}),
                ]
        ...
        assert schema == {
            ...
            "properties": collections.OrderedDict(
                [
                    (
                        "timestamp",
                        {"type": "big_int", "timeKey": True, "path": '"timestamp"'},
                    ),
                    ("host", {"type": "string", "path": '"host"'}),
                ]
            ),
        ...
        assert new_schema == {
            ...
            "properties": collections.OrderedDict(
                [
                    ("time", {"type": "big_int", "expression": '"timestamp" * 1000'}),
                    (
                        "timestamp",
                        {
                            "type": "timestamp",
                            "expression": 'CAST(FROM_UNIXTIME("timestamp") AS timestamp)',
                        },
                    ),
                    ("host", {"type": "string", "path": '"host"'}),
                ]
        ...
        assert new_schema == {
            ...
            "properties": collections.OrderedDict(
                [
                    ("time", {"type": "big_int", "expression": '"timestamp"'}),
                    (
                        "timestamp",
                        {
                            "type": "timestamp",
        ...
        assert new_schema == {
            ...
            "properties": collections.OrderedDict(
                [
                    (
                        "time",
                        {"type": "big_int", "expression": '"timestamp"/1000 * 1000'},
                    ),
                    (
                        "timestamp",
                        {
        ...
        assert schema == {
            ...
            "properties": collections.OrderedDict(
                [
                    (
                        "timestamp",
                        {"type": "integer", "timeKey": True, "path": '"timestamp"'},
                    ),
                    ("host", {"type": "string", "path": '"host"'}),
                ]
            ),
        ...
        schema["properties"]["timestamp"] = {
            "type": "object",
            "properties": {
                "rt": {"type": "big_int", "timeKey": True, "format": "epoch_millis"}
            },
        }
        schema["properties"]["host"] = {"type": "string"}
        new_schema = table_metadata_generator.add_time_key(schema=schema)
        ...
        assert new_schema == {
            ...
            "properties": collections.OrderedDict(
                [
                    ("time", {"type": "big_int", "expression": '"timestamp"."rt"'}),
                    (
                        "timestamp",
                        {
                            "type": "timestamp",
                            "expression": 'CAST(FROM_UNIXTIME("timestamp"."rt" / 1000) AS timestamp)',
                        },
                    ),
                    ("host", {"type": "string", "path": '"host"'}),
                ]
    ...
    def test_add_default_partition(
        self,
    ):
        from pipeline_resources_builder.lambda_function import TableMetaDataGenerator
        ...
        assert schema == {
            "type": "object",
            "properties": collections.OrderedDict([("host", {"type": "string"})]),
        }
        assert new_schema == {
            "type": "object",
            "properties": collections.OrderedDict(
                [
                    ...
                    (
                        "__execution_name__",
                        {"type": "string", "partition": True, "expression": "'{{}}'"},
                    ),
                ]
            ),
        }
        new_schema = table_metadata_generator.add_default_partition(
            schema=schema, source_type="fluent-bit"
        )
        assert schema == {
            "type": "object",
        ...
        assert schema == {
            "type": "object",
            "properties": collections.OrderedDict(
                [("host", {"type": "string"}), ("event_hour", {"type": "string"})]
            ),
        }
        assert new_schema == {
            "type": "object",
        ...
        assert schema == {
            ...
            "properties": collections.OrderedDict(
                [
                    ("timestamp", {"type": "integer", "timeKey": True}),
                    ("host", {"type": "string", "partition": True}),
                ]
            ),
        }
                ...
                [
                    ...
                    (
                        "event_hour",
                        {
                            "type": "string",
                            "partition": True,
                            "expression": "date_format(CAST(FROM_UNIXTIME(\"timestamp\") AS timestamp), '%Y%m%d%H')",
                        },
                    ),
                    (
                        "__execution_name__",
        ...
        assert schema == {
            ...
            "properties": collections.OrderedDict(
                [
                    (
                        "timestamp",
                        {"type": "big_int", "timeKey": True, "format": "epoch_millis"},
                    ),
                    ("host", {"type": "string", "partition": True}),
                ]
            ),
        ...
        assert schema == {
            ...
            "properties": collections.OrderedDict(
                [
                    (
                        "timestamp",
                        {"type": "big_int", "timeKey": True, "format": "epoch_second"},
                    ),
                    ("host", {"type": "string", "partition": True}),
                ]
            ),
    ...
    def test_get_raw_table_metadata(self):
        ...
        table_metadata = table_metadata_generator.get_raw_table_metadata(
            schema=schema,
            ...
            "properties": {
                ...
                "kubernetes": {
                    ...
                    "properties": {
                        ...
                        "namespace_name": {
                            "type": "string",
                            "path": '"kubernetes"."namespace_name"',
                        },
                        "container_name": {
                            "type": "string",
        ...
        assert table_metadata.schema == {
            "type": "object",
            "properties": {"host": {"type": "string", "path": '"host"'}},
        }
        assert table_metadata.ignore_partition is True
    ...
    def test_get_parquet_table_metadata_from_raw(self):
        ...
        table_metadata = table_metadata_generator.get_parquet_table_metadata_from_raw(
            schema=schema
        )
        assert isinstance(table_metadata, TableMetaData) is True
        assert table_metadata.schema == {
                    ...
                    (
                        "agent_info",
                        {
                            ...
                            "properties": {
                                ...
                                "az": {"type": "string", "path": '"agent_info"."az"'},
                                "ec2_instance_id": {
                                    "type": "string",
                                ...
                                "kubernetes": {
                                    ...
                                    "properties": {
                                        "pod_name": {
                                            "type": "string",
                                            "path": '"agent_info"."kubernetes"."pod_name"',
                                        },
                                        "namespace_name": {
                                            "type": "string",
                                            "path": '"agent_info"."kubernetes"."namespace_name"',
                                        },
                                        "container_name": {
                                            "type": "string",
                                            "path": '"agent_info"."kubernetes"."container_name"',
                                        },
                                        "docker_id": {
                                            "type": "string",
                                            "path": '"agent_info"."kubernetes"."docker_id"',
                                        },
                                    },
        ...
        assert table_metadata.schema == {
            ...
            "properties": collections.OrderedDict(
                [
                    ...
                    (
                        "service",
                        {"type": "string", "partition": True, "path": '"service"'},
                    ),
                    (
                        "__execution_name__",
                        {
    ...
    def test_get_table_metadata(
        self, mock_s3_context, mock_sqs_context, mock_iam_context, mock_ddb_context
    ):
        from pipeline_resources_builder.lambda_function import (
            Parameters,
            TableMetaDataGenerator,
        ...
        create_application_pipeline_event = {
            "RequestType": "Create",
            "ResourceProperties": {
                "Resource": "pipeline",
        ...
        table_metadata_generator = TableMetaDataGenerator(
            source=param.pipeline_info.source,
            destination=param.pipeline_info.destination,
        )
        raw, parquet, metrics = table_metadata_generator.get_table_metadata()
        ...
        assert raw.schema == {
            ...
            "properties": {
                ...
                "correlationId": {"type": "string", "path": '"correlationId"'},
                "processInfo": {
                    "type": "object",
                    "properties": {
                        "hostname": {
            ...
            "properties": {
                ...
                "agent_info": {
                    ...
                    "properties": {
                        ...
                        "cluster": {"type": "string", "path": '"agent_info"."cluster"'},
                        "kubernetes": {
                            "type": "object",
                            "properties": {
                                "pod_name": {
        ...
        assert parquet.schema == {
            ...
            "properties": {
                ...
                "elb": {"type": "string", "partition": True, "path": '"elb"'},
                "client_ip": {"type": "string", "path": '"client_ip"'},
                "client_port": {"type": "integer", "path": '"client_port"'},
                "target_ip": {"type": "string", "path": '"target_ip"'},
                "target_port": {"type": "integer", "path": '"target_port"'},
                "event_hour": {
                    "type": "string",
                    "partition": True,
                    "expression": "date_format(parse_datetime(\"timestamp\", 'YYYY-MM-dd''T''HH:mm:ss.SSSSSSSSSZ'), '%Y%m%d%H')",
    ...
    def test_get_table_metadata_and_statements_waf(
        self, mock_s3_context, mock_iam_context, mock_sqs_context, mock_ddb_context
    ):
        from pipeline_resources_builder.lambda_function import (
            Parameters,
        ...
        raw, parquet, metrics, statements = (
            table_metadata_generator.get_table_metadata_and_statements()
        )
        assert raw is SOURCE_TYPE_RESOURCES["waf"]["schema"][0]
        assert parquet is SOURCE_TYPE_RESOURCES["waf"]["schema"][1]
        assert metrics is SOURCE_TYPE_RESOURCES["waf"]["schema"][2]
    ...
    def test_get_table_metadata_and_statements_alb(
        self, mock_s3_context, mock_iam_context, mock_sqs_context, mock_ddb_context
    ):
        from pipeline_resources_builder.lambda_function import (
            Parameters,
    ...
    def test_get_table_metadata_and_statements_cloudfront(
        self, mock_s3_context, mock_iam_context, mock_sqs_context, mock_ddb_context
    ):
        from pipeline_resources_builder.lambda_function import (
            Parameters,
    ...
    def test_get_table_metadata_and_statements_cloudtrail(
        self, mock_s3_context, mock_iam_context, mock_sqs_context, mock_ddb_context
    ):
        from pipeline_resources_builder.lambda_function import (
            Parameters,
    ...
    def test_get_table_metadata_and_statements_vpcflow(
        self, mock_s3_context, mock_iam_context, mock_sqs_context, mock_ddb_context
    ):
        from pipeline_resources_builder.lambda_function import (
            Parameters,
    ...
    def test_get_table_metadata_and_statements_application(
        self, mock_s3_context, mock_iam_context, mock_sqs_context, mock_ddb_context
    ):
        from pipeline_resources_builder.lambda_function import (
            Parameters,
        ...
        assert (
            statements.create
            == "CREATE EXTERNAL TABLE IF NOT EXISTS `tmp`.`application{}` (`log` string)  ROW FORMAT SERDE 'org.openx.data.jsonserde.JsonSerDe'  STORED AS INPUTFORMAT 'org.apache.hadoop.mapred.TextInputFormat' OUTPUTFORMAT 'org.apache.hadoop.hive.ql.io.HiveIgnoreKeyTextOutputFormat' LOCATION '{}' ;"
        )
        assert (
            statements.insert
            == """INSERT INTO "centralized"."application" ("log", "event_hour", "__execution_name__") SELECT "log", date_format(now(), '%Y%m%d%H'), '{}' FROM "tmp"."application{}" ;"""
        )
        assert statements.drop == "DROP TABLE IF EXISTS `tmp`.`application{}`"
        assert statements.aggregate == []
...
class TestPipelineResourceBuilder:
    ...
    def init_default_parameter(
        ...
        self.s3_public_access_policy_arn = (
            f"arn:aws:iam::{self.account_id}:policy/{self.s3_public_access_policy}"
        )
        ...
        self.replication_function_arn = os.environ[
        ...
        self.log_processor_start_execution_role = (
            f"arn:aws:iam::{self.account_id}:role/{self.log_processor_name}"
        )
        self.log_merger_start_execution_role = (
            f"arn:aws:iam::{self.account_id}:role/{self.log_merger_name}"
        )
        self.log_archive_start_execution_role = (
            f"arn:aws:iam::{self.account_id}:role/{self.log_archive_name}"
        )
        self.table_name = "waf"
        self.sts_role = ""
        ...
        self.pipeline_info = {
            "data": {
                ...
                "staging": {"prefix": self.staging_bucket_prefix},
            },
            "type": "Pipeline",
            "stack": {
                "lambda": {"replicate": self.replication_function_arn},
                "queue": {
                    "logEventQueue": self.replication_sqs_arn,
                    "logEventDLQ": self.replication_dlq_arn,
                },
                "role": {"replicate": self.replication_role_arn},
            },
        }
        self.pipeline_event = {
            "RequestType": "Create",
            "ResourceProperties": {"Resource": "pipeline", "Id": self.pipeline_id},
        }
        ...
        self.ingestion_event = {
            "RequestType": "Create",
            "ResourceProperties": {"Resource": "ingestion", "Id": self.ingestion_id},
        }
        ...
        httpserver.expect_request(
            uri=f"/api/datasources/name/Athena-clo-{self.account_id}-{self.aws_region}",
            method="GET",
        ).respond_with_data(
            response_data=json.dumps(
                {
                    "id": 96,
                    "name": f"Athena-clo-{self.account_id}-{self.aws_region}",
                    "uid": "g5Aeh9_Vk",
                    "type": "grafana-athena-datasource",
                }
            )
        ...
        httpserver.expect_request(
            uri=f"/api/dashboards/uid/{self.pipeline_id}-00", method="GET"
        ).respond_with_data(
            response_data=json.dumps({"message": "Dashboard not found", "traceID": ""}),
            status=404,
        )
        httpserver.expect_request(
            uri=f"/api/dashboards/uid/{self.pipeline_id}-01", method="GET"
        ).respond_with_data(
            response_data=json.dumps({"message": "Dashboard not found", "traceID": ""}),
            status=404,
        )
        ...
        self.pipeline_resource_builder = PipelineResourceBuilder(
            parameters=self.pipeline_parameters
        )

        self.ingestion_parameters = Parameters(self.ingestion_event)
        self.ingestion_resource_builder = PipelineResourceBuilder(
            parameters=self.ingestion_parameters
        )
    ...
    def test_init(
        ...
        assert (
            isinstance(self.pipeline_resource_builder.ddb_client_metadata, MetaTable)
            is True
        )
        assert (
            isinstance(self.pipeline_resource_builder.scheduler_client, SchedulerClient)
            is True
        )
        assert (
            isinstance(self.pipeline_resource_builder.events_client, EventsClient)
            is True
        )
        assert isinstance(self.pipeline_resource_builder.sqs_client, SQSClient) is True
        assert (
            isinstance(self.pipeline_resource_builder.glue_client, GlueClient) is True
        )
        assert isinstance(self.pipeline_resource_builder.iam_client, IAMClient) is True

    def test_table_metadata_and_statements(
        self,
        httpserver: HTTPServer,
        mock_iam_context,
        mock_s3_context,
        ...
        table_metadat_and_statements = (
            self.pipeline_resource_builder.table_metadata_and_statements
        )
        ...
        assert (
            isinstance(table_metadat_and_statements.statements, types.SimpleNamespace)
            is True
        )

    def test_init_grafana_info(
        self,
        httpserver: HTTPServer,
        mock_iam_context,
        mock_s3_context,
        ...
        pipeline_event = {
            "RequestType": "Create",
            "ResourceProperties": {"Resource": "pipeline", "Id": pipeline_id},
        }
        ...
        assert (
            isinstance(pipeline_resource_builder.grafana_client, GrafanaClient) is True
        )
    ...
    def test_get_grafana_dashboard(
        self,
        httpserver: HTTPServer,
        mock_iam_context,
        mock_s3_context,
        ...
        from pipeline_resources_builder.logschema import (
            ALB_GRAFANA_DETAILS,
            ALB_GRAFANA_DASHBOARD,
        )
        from pipeline_resources_builder.logschema import (
            WAF_GRAFANA_DETAILS,
            WAF_GRAFANA_DASHBOARD,
        )
        from pipeline_resources_builder.logschema import (
            CLOUDFRONT_GRAFANA_DETAILS,
            CLOUDFRONT_GRAFANA_DASHBOARD,
        )
        from pipeline_resources_builder.logschema import (
            CLOUDTRAIL_GRAFANA_DETAILS,
            CLOUDTRAIL_GRAFANA_DASHBOARD,
        )
        from pipeline_resources_builder.logschema import (
            VPCFLOW_GRAFANA_DETAILS,
            VPCFLOW_GRAFANA_DASHBOARD,
        )
        ...
        details, dashboards = self.pipeline_resource_builder.get_grafana_dashboard(
            source_type="alb"
        )
    ...
    def test_import_dashboards_into_grafana(
        self,
        httpserver: HTTPServer,
        mock_iam_context,
        mock_s3_context,
        ...
        httpserver.expect_oneshot_request(
            uri=f"/api/dashboards/uid/{pipeline_id}-00", method="GET"
        ).respond_with_data(
            response_data=json.dumps({"message": "Dashboard not found", "traceID": ""}),
            status=404,
        )
        httpserver.expect_oneshot_request(
            uri=f"/api/dashboards/uid/{pipeline_id}-01", method="GET"
        ).respond_with_data(
            response_data=json.dumps({"message": "Dashboard not found", "traceID": ""}),
            status=404,
        )
        ...
        pipeline_info = copy.deepcopy(self.pipeline_info)
        pipeline_info["data"]["source"]["type"] = "s3"
        pipeline_info["data"]["source"]["table"][
            "schema"
        ] = '{"type": "objects","properties":{"time":{"type":"string"}}}'
    ...
    def test_delete_dashboards_from_grafana(
        self,
        httpserver: HTTPServer,
        mock_iam_context,
        mock_s3_context,
        ...
        httpserver.expect_request(
            uri=f"/api/dashboards/uid/{pipeline_id}-00", method="DELETE"
        ).respond_with_data(
            response_data=json.dumps(
                {
                    "id": 17,
        ...
        httpserver.expect_request(
            uri=f"/api/dashboards/uid/{pipeline_id}-01", method="DELETE"
        ).respond_with_data(
            response_data=json.dumps(
                {
                    "id": 17,
        ...
        ).respond_with_data(response_data="{}", status=401)
        httpserver.expect_request(
            uri=f"/api/dashboards/uid/{pipeline_id}-00", method="DELETE"
        ).respond_with_data(
            response_data=json.dumps(
    ...
    def test_create_glue_table(
        ...
        response = AWS_GLUE.get_table(
            database=self.centralized_database, name="waf_metrics"
        )
        assert response["Table"]["DatabaseName"] == self.centralized_database
        ...
        response = AWS_GLUE.get_table(
            database=self.centralized_database, name="application"
        )
        assert response["Table"]["DatabaseName"] == self.centralized_database
        assert response["Table"]["Name"] == "application"

        response = AWS_GLUE.get_table(
            database=self.centralized_database, name="application_metrics"
        )
        assert response == {}

    def test_update_glue_table(
        self,
        httpserver: HTTPServer,
        mock_iam_context,
        mock_s3_context,
    ...
    def test_create_rule(
        ...
        pipeline_info["data"]["source"]["type"] = "waf"
        pipeline_info["data"]["destination"]["table"]["name"] = "waf"
        pipeline_info["data"]["scheduler"]["LogProcessor"][
            "schedule"
        ] = "rate(5 minutes)"
        pipeline_info["data"]["scheduler"]["LogMerger"][
            "schedule"
        ] = "cron(0 1 * * ? *)"
        pipeline_info["data"]["scheduler"]["LogArchive"][
            "schedule"
        ] = "cron(0 2 * * ? *)"
        pipeline_event = {
            "RequestType": "Create",
        ...
        response = AWS_EVENTS._events_client.list_rules(
            NamePrefix=f"LogProcessor-{pipeline_id}", EventBusName="default"
        )
        ...
        response = AWS_EVENTS._events_client.list_targets_by_rule(
            Rule=f"LogProcessor-{pipeline_id}", EventBusName="default"
        )
        assert response["Targets"][0]["Id"] == pipeline_id

        response = AWS_EVENTS._events_client.list_rules(
            NamePrefix=f"LogMerger-{pipeline_id}", EventBusName="default"
        )
        ...
        response = AWS_EVENTS._events_client.list_targets_by_rule(
            Rule=f"LogMerger-{pipeline_id}", EventBusName="default"
        )
        assert response["Targets"][0]["Id"] == pipeline_id

        response = AWS_EVENTS._events_client.list_rules(
            NamePrefix=f"LogMergerForMetrics-{pipeline_id}", EventBusName="default"
        )
        ...
        response = AWS_EVENTS._events_client.list_targets_by_rule(
            Rule=f"LogMergerForMetrics-{pipeline_id}", EventBusName="default"
        )
        assert response["Targets"][0]["Id"] == pipeline_id

        response = AWS_EVENTS._events_client.list_rules(
            NamePrefix=f"LogArchive-{pipeline_id}", EventBusName="default"
        )
        ...
        response = AWS_EVENTS._events_client.list_targets_by_rule(
            Rule=f"LogArchive-{pipeline_id}", EventBusName="default"
        )
        assert response["Targets"][0]["Id"] == pipeline_id

        response = AWS_EVENTS._events_client.list_rules(
            NamePrefix=f"LogArchiveForMetrics-{pipeline_id}", EventBusName="default"
        )
    ...
    def test_delete_rule(
        ...
        response = AWS_EVENTS._events_client.list_targets_by_rule(
            Rule=f"LogArchiveForMetrics-{pipeline_id}", EventBusName="default"
        )
        assert response["Targets"][0]["Id"] == pipeline_id
        ...
        response = AWS_EVENTS._events_client.list_rules(
            NamePrefix=f'LogProcessor-{pipeline_info["data"]["destination"]["table"]["name"]}',
            EventBusName="default",
        )
        assert response["Rules"] == []

        response = AWS_EVENTS._events_client.list_rules(
            NamePrefix=f'LogMerger-{pipeline_info["data"]["destination"]["table"]["name"]}',
            EventBusName="default",
        )
        assert response["Rules"] == []

        response = AWS_EVENTS._events_client.list_rules(
            NamePrefix=f'LogMergerForMetrics-{pipeline_info["data"]["destination"]["table"]["name"]}',
            EventBusName="default",
        )
        assert response["Rules"] == []

        response = AWS_EVENTS._events_client.list_rules(
            NamePrefix=f'LogArchive-{pipeline_info["data"]["destination"]["table"]["name"]}',
            EventBusName="default",
        )
        assert response["Rules"] == []

        response = AWS_EVENTS._events_client.list_rules(
            NamePrefix=f'LogArchiveForMetrics-{pipeline_info["data"]["destination"]["table"]["name"]}',
            EventBusName="default",
        )
        assert response["Rules"] == []
    ...
    def test_create_scheduler(
        ...
        response = AWS_SCHEDULER.get_schedule(
            name="LogProcessor", group_name=pipeline_id
        )
        assert (
            response["Arn"]
            == f"arn:aws:scheduler:{self.aws_region}:{self.account_id}:schedule/{pipeline_id}/LogProcessor"
        )
        ...
        assert (
            response["Arn"]
            == f"arn:aws:scheduler:{self.aws_region}:{self.account_id}:schedule/{pipeline_id}/LogMerger"
        )
        ...
        response = AWS_SCHEDULER.get_schedule(
            name="LogMergerForMetrics", group_name=pipeline_id
        )
        assert (
            response["Arn"]
            == f"arn:aws:scheduler:{self.aws_region}:{self.account_id}:schedule/{pipeline_id}/LogMergerForMetrics"
        )
        assert response["FlexibleTimeWindow"] == {
            "MaximumWindowInMinutes": 30,
            "Mode": "FLEXIBLE",
        ...
        assert (
            response["Arn"]
            == f"arn:aws:scheduler:{self.aws_region}:{self.account_id}:schedule/{pipeline_id}/LogArchive"
        )
        assert response["FlexibleTimeWindow"] == {
            "MaximumWindowInMinutes": 30,
            "Mode": "FLEXIBLE",
        ...
        response = AWS_SCHEDULER.get_schedule(
            name="LogArchiveForMetrics", group_name=pipeline_id
        )
        assert (
            response["Arn"]
            == f"arn:aws:scheduler:{self.aws_region}:{self.account_id}:schedule/{pipeline_id}/LogArchiveForMetrics"
        )
        assert response["FlexibleTimeWindow"] == {
            "MaximumWindowInMinutes": 30,
            "Mode": "FLEXIBLE",
    ...
    def test_create_connector(
        ...
        response = AWS_SCHEDULER.get_schedule(
            name=f"Connector-{ingestion_id}", group_name=pipeline_id
        )
        assert (
            response["Arn"]
            == f"arn:aws:scheduler:{self.aws_region}:{self.account_id}:schedule/{pipeline_id}/Connector-{ingestion_id}"
        )
        assert response["FlexibleTimeWindow"] == {"Mode": "OFF"}
        assert response["GroupName"] == pipeline_id
        ...
        response = AWS_EVENTS._events_client.list_rules(
            NamePrefix=f"Connector-{ingestion_id}", EventBusName="default"
        )
        ...
        response = AWS_EVENTS._events_client.list_targets_by_rule(
            Rule=f"Connector-{ingestion_id}", EventBusName="default"
        )
        ...
        ingestion_info["data"]["source"]["context"] = json.dumps(
            {
                ...
                "role": f"{self.replication_role_arn}-source",
            }
        )
        ...
        assert AWS_IAM.get_role_policy(
            role_name=self.log_processor_start_execution_role.split("/")[-1],
            policy_name=parameters.policy_sid,
        )["PolicyDocument"] == {
            "Version": "2012-10-17",
            "Statement": [
                {
                    ...
                    "Resource": [
                        ...
                        f"{self.replication_role_arn}-source",
                    ],
                }
            ],
        }
    ...
    def test_delete_connector(
        ...
        response = AWS_EVENTS._events_client.list_rules(
            NamePrefix=f"Connector-{pipeline_id}", EventBusName="default"
        )
        assert response["Rules"] == []

    def test_init_destination_policy_info(
        self,
        httpserver: HTTPServer,
        mock_iam_context,
        mock_s3_context,
        ...
        response = AWS_IAM._iam_client.create_policy(
            PolicyName="TestPolicy1", PolicyDocument=json.dumps(policy_document)
        )
        ...
        assert (
            pipeline_resource_builder.s3_public_access_policy_sid
            == "S3AccessPolicyForDestination"
        )
        assert (
            pipeline_resource_builder.s3_public_access_policy_document["Document"][
                "Statement"
            ]
            == []
        )
        ...
        assert pipeline_resource_builder.s3_public_access_policy_document["Document"][
            "Statement"
        ] == [
            {
                "Sid": "S3AccessPolicyForDestination",
        ...
        response = AWS_IAM._iam_client.create_policy(
            PolicyName="TestPolicy4", PolicyDocument=json.dumps(policy_document)
        )
        test_policy_arn = response["Policy"]["Arn"]
    ...
    def test_grant_destination_bucket_access_permission(
        self,
        httpserver: HTTPServer,
        mock_iam_context,
        mock_s3_context,
        ...
        response = AWS_IAM.get_policy_document(
            arn=pipeline_resource_builder.parameters.pipeline_info.role.s3_public_access,
            sid=pipeline_resource_builder.s3_public_access_policy_sid,
        )
        ...
        response = AWS_IAM._iam_client.create_policy(
            PolicyName="TestPolicy", PolicyDocument=json.dumps(policy_document)
        )
        test_policy_arn = response["Policy"]["Arn"]
    ...
    def test_revoke_destination_bucket_access_permission(
        self,
        httpserver: HTTPServer,
        mock_iam_context,
        mock_s3_context,
    ...
    def test_grant_sqs_send_message_permission_to_source_bucket(
        self,
        httpserver: HTTPServer,
        mock_iam_context,
        mock_s3_context,
        ...
        assert response["Statement"] == [
            {
                ...
                "Condition": {
                    "ArnLike": {"aws:SourceArn": "arn:aws:s3:::logging-bucket"}
                },
    ...
    def test_revoke_sqs_send_message_permission(
        self,
        httpserver: HTTPServer,
        mock_iam_context,
        mock_s3_context,
    ...
    def test_init_send_email_via_sns_policy_info(
        self,
        httpserver: HTTPServer,
        mock_iam_context,
        mock_s3_context,
        ...
        send_template_email_sns_public_policy_arn = os.environ[
        ...
        assert (
            pipeline_resource_builder.send_email_via_sns_policy_arn
            == send_template_email_sns_public_policy_arn
        )

    def test_grant_publish_sns_message_permission(
        self,
        httpserver: HTTPServer,
        mock_iam_context,
        mock_s3_context,
        ...
        assert AWS_IAM.get_policy_document(
            arn=send_template_email_sns_public_policy_arn,
    ...
    def test_revoke_publish_sns_message_permission(
        self,
        httpserver: HTTPServer,
        mock_iam_context,
        mock_s3_context,
    ...
    def test_grant_source_bucket_access_permission(
        self,
        httpserver: HTTPServer,
        mock_iam_context,
        mock_s3_context,
        ...
        ingestion_resource_builder = PipelineResourceBuilder(
            parameters=ingestion_parameters
        )
        ingestion_resource_builder.s3_client = S3Client(
            sts_role_arn=ingestion_parameters.ingestion_info.role.sts
        )

        ingestion_resource_builder.grant_source_bucket_access_permission()

        response = AWS_S3.get_bucket_policy(
            bucket=ingestion_parameters.ingestion_info.source.location.bucket,
        ...
        response = AWS_IAM.get_role_policy(
            role_name=ingestion_parameters.pipeline_info.role.replicate.split("/")[-1],
            policy_name=ingestion_parameters.policy_sid,
        )
        assert (
            response["RoleName"]
            == ingestion_parameters.pipeline_info.role.replicate.split("/")[-1]
        )
        ...
        application_ingestion_info = AWS_DDB_META.get(
            meta_name=application_ingestion_id
        )
        application_ingestion_info["data"]["role"][
            "sts"
        ] = log_processor_start_execution_role_arn
        ...
        assert response["PolicyDocument"] == {
            ...
            "Statement": [
                ...
                },
                {
                    "Effect": "Allow",
                    "Action": "sts:AssumeRole",
                    "Resource": [log_processor_start_execution_role_arn],
                },
            ],
        }

    def test_revoke_source_bucket_access_permission(
        self,
        httpserver: HTTPServer,
        mock_iam_context,
        mock_s3_context,
    ...
    def test_put_event_notification_into_source_bucket(
        self,
        httpserver: HTTPServer,
        mock_iam_context,
        mock_s3_context,
        ...
        response = AWS_S3.get_bucket_notification(
            bucket=ingestion_parameters.ingestion_info.source.location.bucket
        )
        assert response == {
            "QueueConfigurations": [
        ...
        assert response["Statement"] == [
            {
                ...
                "Principal": {"AWS": self.replication_role_arn},
                "Resource": [
                    f"arn:aws:s3:::{self.logging_bucket_name}/{os.path.normpath(self.logging_bucket_prefix)}*"
                ],
                "Sid": ingestion_id.replace("-", ""),
            },
        ]
        ...
        response = ingestion_resource_builder.events_client._events_client.list_rules(
            NamePrefix=f"S3EventDriver-{ingestion_parameters.ingestion_id}",
            EventBusName="default",
        )
        assert response["Rules"][0] == {
            "Name": f"S3EventDriver-{ingestion_parameters.ingestion_id}",
            "Arn": f"arn:aws:events:{self.aws_region}:{self.account_id}:rule/S3EventDriver-{ingestion_parameters.ingestion_id}",
            "State": "ENABLED",
            "EventPattern": json.dumps(
                {
                    ...
                    "detail": {
                        "bucket": {"name": [self.logging_bucket_name]},
                        "object": {
                            "key": [
                                {"prefix": os.path.normpath(self.logging_bucket_prefix)}
                            ]
                        },
                    },
                }
        ...
        response = ingestion_resource_builder.events_client._events_client.list_targets_by_rule(
            Rule=f"S3EventDriver-{ingestion_parameters.ingestion_id}",
            EventBusName="default",
        )
        assert response["Targets"][0]["Id"] == ingestion_parameters.ingestion_id
        assert response["Targets"][0]["Arn"] == self.replication_function_arn
        assert "RoleArn" not in response["Targets"][0]
        assert response["Targets"][0].get("Input") is None

    def test_delete_event_notification_from_source_bucket(
        self,
        httpserver: HTTPServer,
        mock_iam_context,
        mock_s3_context,
        ...
        response = AWS_S3.get_bucket_notification(
            bucket=self.ingestion_parameters.ingestion_info.source.location.bucket
        )
        assert response == {}
        ...
        response = AWS_EVENTS._events_client.list_rules(
            NamePrefix=f"S3EventDriver-{ingestion_id}", EventBusName="default"
        )
        assert response["Rules"] == []

    def test_execute_all_ingestion_operations_in_pipeline_in_bulk(
        self,
        httpserver: HTTPServer,
        mock_iam_context,
        mock_s3_context,
        ...
        parameters = Parameters(pipeline_event)
        pipeline_resource_builder = PipelineResourceBuilder(parameters=parameters)
        pipeline_resource_builder.execute_all_ingestion_operations_in_pipeline_in_bulk(
            pipeline_id=pipeline_id, action="create"
        )
        ...
        pipeline_resource_builder.execute_all_ingestion_operations_in_pipeline_in_bulk(
            pipeline_id=pipeline_id, action="delete"
        )
    ...
    def test_create_pipeline(
        ...
        assert (
            exception_info.value.args[0]
            == "Pipeline Id: do-not-exists Information is not exist in Meta Table."
        )
        ...
        pipeline_resource_builder = PipelineResourceBuilder(
        ...
        assert (
            ...
            == f"s3://{self.centralized_bucket_name}/{self.centralized_bucket_prefix}/{self.centralized_database}/{self.table_name}"
        )
        assert (
        ...
        response = AWS_GLUE.get_partition_indexes(
            database=self.centralized_database, table_name=self.table_name
        )
        assert response["PartitionIndexDescriptorList"] == []

        response = AWS_GLUE.get_table(
            database=self.centralized_database, name=f"{self.table_name}_metrics"
        )
        ...
        assert (
            response["Table"]["StorageDescriptor"]["Location"]
            == f"s3://{self.centralized_bucket_name}/{self.centralized_bucket_prefix}/{self.centralized_database}/{self.table_name}_metrics"
        )
        assert (
            response["Table"]["StorageDescriptor"]["InputFormat"]
            == "org.apache.hadoop.hive.ql.io.parquet.MapredParquetInputFormat"
        ...
        response = AWS_GLUE.get_partition_indexes(
            database=self.centralized_database, table_name=f"{self.table_name}_metrics"
        )
        assert response["PartitionIndexDescriptorList"] == []
        ...
        assert (
            ...
            == pipeline_parameters.pipeline_info.scheduler.log_processor.arn
        )
        assert (
            response["Targets"][0]["RoleArn"]
            == pipeline_parameters.pipeline_info.scheduler.log_processor.execution_role
        )
        ...
        assert (
            response["Targets"][0]["Arn"]
            == pipeline_parameters.pipeline_info.scheduler.log_merger.arn
        )
        assert (
            response["Targets"][0]["RoleArn"]
            == pipeline_parameters.pipeline_info.scheduler.log_merger.execution_role
        )
        assert json.loads(response["Targets"][0]["Input"]) == {
            "metadata": {
                "pipelineId": pipeline_id,
        ...
        assert (
            response["Targets"][0]["Arn"]
            == pipeline_parameters.pipeline_info.scheduler.log_archive.arn
        )
        assert (
            response["Targets"][0]["RoleArn"]
            == pipeline_parameters.pipeline_info.scheduler.log_archive.execution_role
        )
        assert json.loads(response["Targets"][0]["Input"]) == {
            "metadata": {
                "pipelineId": pipeline_id,
        ...
        assert (
            response["Arn"]
            == f"arn:aws:scheduler:{self.aws_region}:{self.account_id}:schedule-group/{pipeline_id}"
        )
        ...
        self.logging_bucket_prefix = (
            f"/AWSLogs/{self.account_id}/{self.aws_region}/WAFLogs/"
        )
        self.processor_schedule = "rate(5 minutes)"
        ...
        pipeline_info = copy.deepcopy(self.pipeline_info)
        pipeline_info["data"]["destination"]["table"]["name"] = self.table_name
        pipeline_info["data"]["destination"]["location"][
            "bucket"
        ] = centralized_bucket_name
        pipeline_info["data"]["notification"]["service"] = self.notification_service
        pipeline_info["data"]["notification"]["recipients"] = self.recipients
        pipeline_info["data"]["scheduler"]["service"] = "scheduler"
        pipeline_info["data"]["scheduler"]["LogProcessor"][
        ...
        assert (
            response["Table"]["StorageDescriptor"]["Location"]
            == f"s3://{centralized_bucket_name}/{self.centralized_bucket_prefix}/{self.centralized_database}/{self.table_name}"
        )
        assert (
            response["Table"]["StorageDescriptor"]["InputFormat"]
            == "org.apache.hadoop.hive.ql.io.parquet.MapredParquetInputFormat"
        ...
        assert (
            response["Table"]["StorageDescriptor"]["Location"]
            == f"s3://{centralized_bucket_name}/{self.centralized_bucket_prefix}/{self.centralized_database}/{self.table_name}_metrics"
        )
        assert (
            response["Table"]["StorageDescriptor"]["InputFormat"]
            == "org.apache.hadoop.hive.ql.io.parquet.MapredParquetInputFormat"
        ...
        assert AWS_IAM.get_policy_document(
            arn=send_template_email_sns_public_policy_arn,
            sid=pipeline_parameters.policy_sid,
        )["Document"] == {
            "Statement": [
                {
                    "Action": "SNS:Publish",
        ...
        pipeline_info["data"]["notification"][
            "recipients"
        ] = "arn:aws:iam::123456789012:policy/not-a-sns-arn"
        pipeline_event = {
            "RequestType": "Create",
            "ResourceProperties": {"Resource": "pipeline", "Id": pipeline_id},
        }
    ...
    def test_delete_pipeline(
        ...
        pipeline_event = {
            "RequestType": "Create",
            "ResourceProperties": {"Resource": "pipeline", "Id": "do-not-exists"},
        }
        parameters = Parameters(pipeline_event)
        pipeline_resource_builder = PipelineResourceBuilder(parameters=parameters)
        ...
        PipelineResourceBuilder(
            parameters=Parameters(create_pipeline_event)
        ).create_pipeline()
        PipelineResourceBuilder(
            parameters=Parameters(create_ingestion_event)
        ).create_ingestion()
        PipelineResourceBuilder(
            parameters=Parameters(delete_pipeline_event)
        ).delete_pipeline()
        ...
        response = AWS_GLUE.get_table(
            database=self.centralized_database, name=table_name
        )
        assert response == {}

        response = AWS_GLUE.get_table(
            database=self.centralized_database, name=f"{table_name}_metrics"
        )
        assert response == {}
        ...
        assert (
            AWS_IAM.get_policy_document(
                arn=send_template_email_sns_public_policy_arn, sid=parameters.policy_sid
            )["Document"]["Statement"]
            == []
        )
    ...
    def test_create_ingestion(
        ...
        do_not_exists_ingestion_event = {
            "RequestType": "Create",
            "ResourceProperties": {"Resource": "ingestion", "Id": "do-not-exists"},
        }
        ...
        assert (
            exception_info.value.args[0]
            == "Ingestion Id: do-not-exists Information is not exist in Meta Table."
        )
        ...
        ingestion_info["data"]["source"][
            "prefix"
        ] = f"///{self.staging_bucket_prefix}/{ingestion_id}///"
        ingestion_info["pipelineId"] = pipeline_id
        ingestion_event = {
            "RequestType": "Create",
            "ResourceProperties": {"Resource": "ingestion", "Id": ingestion_id},
        ...
        pipeline_info["data"]["scheduler"]["LogArchive"]["age"] = self.archive_age
        pipeline_info["data"]["staging"][
            "prefix"
        ] = f"///{self.staging_bucket_prefix}///"
        ...
        assert response["QueueConfigurations"] == [
            {
                ...
                "Filter": {
                    "Key": {
                        "FilterRules": [
                            {"Name": "prefix", "Value": "not-overlap-prefix"}
                        ]
                    }
                },
            }
        ...
        assert response["QueueConfigurations"] == [
            {
                ...
                "Filter": {
                    "Key": {
                        "FilterRules": [{"Name": "prefix", "Value": "AWSLogs/WAFLogs"}]
                    }
                },
            }
        ]
        ...
        assert response["Statement"] == [
            {
                ...
                "Principal": {"AWS": self.replication_role_arn},
                "Resource": [f"arn:aws:s3:::{self.logging_bucket_name}/*"],
                "Sid": ingestion_id.replace("-", ""),
            },
        ]

        response = AWS_IAM.get_role_policy(
            role_name=self.replication_role_arn.split("/")[-1],
            policy_name=ingestion_id.replace("-", ""),
        )
        assert response["PolicyDocument"] == {
            "Statement": [
                {
...
def test_lambda_handler(
    ...
    s3_public_access_policy_arn = (
        f"arn:aws:iam::{account_id}:policy/{s3_public_access_policy}"
    )
    replication_sqs_arn = os.environ["REPLICATION_SQS_ARN"]
    ...
    log_processor_arn = (
        f"arn:aws:states:{aws_region}:{account_id}:stateMachine:{log_processor_name}"
    )
    log_merger_arn = (
        f"arn:aws:states:{aws_region}:{account_id}:stateMachine:{log_merger_name}"
    )
    log_archive_arn = (
        f"arn:aws:states:{aws_region}:{account_id}:stateMachine:{log_archive_name}"
    )
    log_processor_start_execution_role = (
        f"arn:aws:iam::{account_id}:role/{log_processor_name}"
    )
    log_merger_start_execution_role = (
        f"arn:aws:iam::{account_id}:role/{log_merger_name}"
    )
    log_archive_start_execution_role = (
        f"arn:aws:iam::{account_id}:role/{log_archive_name}"
    )
    table_name = "waf"
    sts_role = ""
    ...
    assert (
        response["Table"]["StorageDescriptor"]["Location"]
        == f"s3://{centralized_bucket_name}/{centralized_bucket_prefix}/{centralized_database}/{table_name}"
    )
    assert (
        response["Table"]["StorageDescriptor"]["InputFormat"]
        == "org.apache.hadoop.hive.ql.io.parquet.MapredParquetInputFormat"
    ...
    response = AWS_GLUE.get_partition_indexes(
        database=centralized_database, table_name=table_name
    )
    assert response["PartitionIndexDescriptorList"] == []

    response = AWS_GLUE.get_table(
        database=centralized_database, name=f"{table_name}_metrics"
    )
    assert response["Table"]["Name"] == f"{table_name}_metrics"
    assert response["Table"]["DatabaseName"] == centralized_database
    assert response["Table"]["StorageDescriptor"]["Columns"] == [
    ...
    assert (
        response["Table"]["StorageDescriptor"]["Location"]
        == f"s3://{centralized_bucket_name}/{centralized_bucket_prefix}/{centralized_database}/{table_name}_metrics"
    )
    assert (
        response["Table"]["StorageDescriptor"]["InputFormat"]
        == "org.apache.hadoop.hive.ql.io.parquet.MapredParquetInputFormat"
    ...
    response = AWS_GLUE.get_partition_indexes(
        database=centralized_database, table_name=f"{table_name}_metrics"
    )
    assert response["PartitionIndexDescriptorList"] == []
    ...
    assert (
        response["Arn"]
        == f"arn:aws:scheduler:{aws_region}:{account_id}:schedule-group/{pipeline_id}"
    )

    response = AWS_SCHEDULER.get_schedule(name="LogProcessor", group_name=pipeline_id)
    assert (
        response["Arn"]
        == f"arn:aws:scheduler:{aws_region}:{account_id}:schedule/{pipeline_id}/LogProcessor"
    )
    assert response["FlexibleTimeWindow"] == {"Mode": "OFF"}
    assert response["GroupName"] == pipeline_id
    ...
    assert (
        response["Arn"]
        == f"arn:aws:scheduler:{aws_region}:{account_id}:schedule/{pipeline_id}/LogMerger"
    )
    assert response["FlexibleTimeWindow"] == {
        "Mode": "FLEXIBLE",
        "MaximumWindowInMinutes": 30,
    ...
    assert (
        response["Arn"]
        == f"arn:aws:scheduler:{aws_region}:{account_id}:schedule/{pipeline_id}/LogMergerForMetrics"
    )
    assert response["FlexibleTimeWindow"] == {
        "Mode": "FLEXIBLE",
        "MaximumWindowInMinutes": 30,
    ...
    assert (
        response["Arn"]
        == f"arn:aws:scheduler:{aws_region}:{account_id}:schedule/{pipeline_id}/LogArchive"
    )
    assert response["FlexibleTimeWindow"] == {
        "Mode": "FLEXIBLE",
        "MaximumWindowInMinutes": 30,
    ...
    assert (
        response["Arn"]
        == f"arn:aws:scheduler:{aws_region}:{account_id}:schedule/{pipeline_id}/LogArchiveForMetrics"
    )
    assert response["FlexibleTimeWindow"] == {
        "Mode": "FLEXIBLE",
        "MaximumWindowInMinutes": 30,
    ...
    assert response["Statement"] == [
        {
            ...
            "Condition": {
                "ArnLike": {"aws:SourceArn": f"arn:aws:s3:::{logging_bucket_name}"}
            },
            "Sid": ingestion_id.replace("-", ""),
        }
    ]
