...
class TestAlb:
    ...
    def test_user_agent(self):
        ...
        assert (
            self.alb_parser.user_agent
            == "Mozilla/5.0 (Macintosh; PPC Mac OS X 10_9_4) AppleWebKit/536.2 (KHTML, like Gecko) Chrome/35.0.847.0 Safari/536.2"
        )
...
class TestCloudFront:
    ...
    def test_is_record(self):
        ...
        cloudfront_parser = CloudFront(
            record="#Fields: date time x-edge-location sc-bytes c-ip cs-method cs(Host) cs-uri-stem sc-status cs(Referer) cs(User-Agent) cs-uri-query cs(Cookie) x-edge-result-type x-edge-request-id x-host-header cs-protocol cs-bytes time-taken x-forwarded-for ssl-protocol ssl-cipher x-edge-response-result-type cs-protocol-version fle-status fle-encrypted-fields c-port time-to-first-byte x-edge-detailed-result-type sc-content-type sc-content-len sc-range-start sc-range-end"
        )
        assert cloudfront_parser.is_record() is False
    ...
    def test_user_agent(self):
        ...
        assert (
            self.cloudfront_parser.user_agent
            == "Mozilla/5.0 (iPad; CPU iPad OS 12_4_8 like Mac OS X) AppleWebKit/532.2 (KHTML, like Gecko) FxiOS/13.9n5321.0 Mobile/11T585 Safari/532.2"
        )
...
class TestEnrichProcessor:
    ...
    def test_init(self):
        ...
        assert (
            ...
            == f"Do not supported source type: do-not-supported. Supported source type: ['cloudfront', 'alb']."
        )
    ...
    def test_process(self):
        ...
        record = 'https 2023-07-04T13:28:28.138531Z app/ALB/nwpiqzrqc67zsbwq 185.249.140.9:1231 10.2.2.174:443 1.5414835421835185 1.8228018060637856 1.1708408317685808 200 200 1525 59997 "GET http://alb.us-east-1.elb.amazonaws.com/Book-10.png HTTP/1.1" "Mozilla/5.0 (Macintosh; PPC Mac OS X 10_9_4) AppleWebKit/536.2 (KHTML, like Gecko) Chrome/35.0.847.0 Safari/536.2" TLS_AES_128_GCM_SHA256 TLSv1.2 arn:aws:elasticloadbalancing:us-east-1:123456789012:targetgroup/app/e240e6889123qdqw "Root=1-5034982-7f2d2ae7a15148ff825e84b9f59a0c68" "alb.us-east-1.elb.amazonaws.com" "session-reused" 0 2023-07-04T13:28:28.138531Z "forward" "-" "-" "10.2.2.176:443" "200" "-" "-"\n'
        assert (
            enrich_processor.process(record=record, enrich_plugins={""})
            == record.rstrip() + " {}\n"
        )
        ...
        record = 'https 2023-07-04T13:28:28.138531Z app/ALB/nwpiqzrqc67zsbwq 185.249.140.9:1231 10.2.2.174:443 1.5414835421835185 1.8228018060637856 1.1708408317685808 200 200 1525 59997 "GET http://alb.us-east-1.elb.amazonaws.com/Book-10.png HTTP/1.1" "Mozilla/5.0 (Macintosh; PPC Mac OS X 10_9_4) AppleWebKit/536.2 (KHTML, like Gecko) Chrome/35.0.847.0 Safari/536.2" TLS_AES_128_GCM_SHA256 TLSv1.2 arn:aws:elasticloadbalancing:us-east-1:123456789012:targetgroup/app/e240e6889123qdqw "Root=1-5034982-7f2d2ae7a15148ff825e84b9f59a0c68" "alb.us-east-1.elb.amazonaws.com" "session-reused" 0 2023-07-04T13:28:28.138531Z "forward" "-" "-" "10.2.2.176:443" "200" "-" "-"\n'
        assert (
            enrich_processor.process(
                record=record, enrich_plugins={"do-not-support", "user_agent"}
            )
            == record.rstrip()
            + ' {"ua_browser":"Chrome","ua_browser_version":"35.0.847","ua_os":"Mac OS X","ua_os_version":"10.9.4","ua_device":"Mac","ua_category":"PC"}\n'
        )
...
def test_lambda_handler(
    ...
    lambda_function.AWS_S3.download_file(
        bucket=staging_bucket_name,
        key=f"{staging_bucket_prefix}/AWSLogs/{account_id}/alb/alb.log.gz",
        filename=f"{tmp_path}/alb.log.gz",
    )
    ...
    lambda_function.AWS_S3.delete_object(
        bucket=staging_bucket_name,
        key=f"{staging_bucket_prefix}/AWSLogs/{account_id}/alb/alb.log.gz",
    )
...
def test_maxminddb(
    ...
    assert (
        alb_parser.enrich_record(enrich_data={}) == f"{alb_log_record.rstrip()} {{}}\n"
    )
    assert (
        alb_parser.enrich_record(enrich_data={"geo_iso_code": "US"})
        == f'{alb_log_record.rstrip()} {{"geo_iso_code":"US"}}\n'
    )
    assert (
        alb_parser.enrich_record(
            enrich_data={"geo_iso_code": "US", "ua_category": "PC"}
        )
        == f'{alb_log_record.rstrip()} {{"geo_iso_code":"US","ua_category":"PC"}}\n'
    )
    ...
    assert (
        cloudfront_parser.enrich_record(enrich_data={})
        == f"{cloudfront_log_record.rstrip()}\t{{}}\n"
    )
    assert (
        cloudfront_parser.enrich_record(enrich_data={"geo_iso_code": "US"})
        == f'{cloudfront_log_record.rstrip()}\t{{"geo_iso_code":"US"}}\n'
    )
    assert (
        cloudfront_parser.enrich_record(
            enrich_data={"geo_iso_code": "US", "ua_category": "PC"}
        )
        == f'{cloudfront_log_record.rstrip()}\t{{"geo_iso_code":"US","ua_category":"PC"}}\n'
    )
    ...
    assert (
        enrich_processor.get_maxminddb_path()
        == "/opt/python/maxminddb/GeoLite2-City.mmdb"
    )
    ...
    assert (
        enrich_processor.get_maxminddb_path()
        == f"{os.path.dirname(os.path.dirname(os.path.abspath(__file__)))}/s3_object_replication/enrichment/maxminddb/GeoLite2-City.mmdb"
    )
    ...
    assert (
        enrich_processor.process(
            record=record, enrich_plugins=set(["geo_ip", "user_agent"])
        )
        == record_enriched
    )
    ...
    assert (
        enrich_processor.process(
            record='https 2023-07-04T13:28:26.138531Z app/ALB/6cv0xw490oh8nq7n 185.176.232.11:15364 10.2.2.175:443 1.9537748743523755 1.9607717664807693 1.3631916131229302 200 200 1521 50675 "GET https://alb.us-east-1.elb.amazonaws.com/Javascript-Master.png HTTP/2.0" "Mozilla/5.0 (iPod; U; CPU iPhone OS 3_0 like Mac OS X; yi-US) AppleWebKit/531.9.3 (KHTML, like Gecko) Version/3.0.5 Mobile/8B117 Safari/6531.9.3" TLS_CHACHA20_POLY1305_SHA256 TLSv1.2 arn:aws:elasticloadbalancing:us-east-1:123456789012:targetgroup/gateway/06409f87b3bad113 "Root=1-8187477-b5b5cdfa33534d589247a5c61de9fe0e" "alb.us-east-1.elb.amazonaws.com" "session-reused" 0 2023-07-04T13:28:26.138531Z "forward" "-" "-" "10.2.2.174:80" "200" "-" "-"\n',
            enrich_plugins=set(["geo_ip", "user_agent"]),
        )
        == enrich_data
    )
    ...
    lambda_function.AWS_S3.download_file(
        bucket=staging_bucket_name,
        key=f"{staging_bucket_prefix}/AWSLogs/{account_id}/cloudfront/cloudfront.log.gz",
        filename=f"{tmp_path}/cloudfront.log.gz",
    )
...
def test_get_ingestion_sts_role(
    mock_sqs_context,
    mock_s3_context,
    mock_iam_context,
    mock_sts_context,
    ...
    from s3_object_replication.lambda_function import get_ingestion_sts_role
    ...
    assert get_ingestion_sts_role(pipeline_id=waf_pipeline_id) == {
        f"{logging_bucket_name}/{os.path.normpath(logging_bucket_prefix)}": ""
    }
    assert get_ingestion_sts_role(pipeline_id="do-not-exists") == {}


def test_get_sts_role_by_object(
    mock_sqs_context,
    mock_s3_context,
    mock_iam_context,
    mock_sts_context,
    ...
    from s3_object_replication.lambda_function import (
        get_sts_role_by_object,
        AWS_DDB_META,
        TTL_CACHE,
    )
    ...
    assert (
        get_sts_role_by_object(
            ...
            key=f"{os.path.normpath(logging_bucket_prefix)}/test.gzip",
        )
        == ""
    )
    TTL_CACHE.clear()
    ...
    AWS_DDB_META.put(
        ...
        item={
            "data": {
                "role": {
                    "sts": replication_role_arn,
                },
                "source": {
                    "bucket": logging_bucket_name,
                    "prefix": "/test/get_sts_role_by_object/",
                },
            },
    ...
    assert (
        get_sts_role_by_object(
            bucket=logging_bucket_name,
            key="test/get_sts_role_by_object/test.gzip",
        )
