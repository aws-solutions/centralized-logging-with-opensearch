...
export interface PipelineFlowProps {
  ...
  readonly eksAgent2AOSPipelineStfnLambdaArn: string;
}
 ...
 */
export class EKSAgent2AOSPipelineFlowStack extends Construct {
  ...
  private updateStatus(
    ...
    ddbTable: ITable,
    status: string,
    extra?: KeyVal<tasks.DynamoAttributeValue>
  ) {
    const base = {
      status: tasks.DynamoAttributeValue.fromString(status),
      stackId: tasks.DynamoAttributeValue.fromString(
  ...
  constructor(scope: Construct, id: string, props: PipelineFlowProps) {
    ...
    //lambda
    const eksAgent2AOSPipelineStfnLambda = Function.fromFunctionArn(
      this,
      "EKSAgent2AOSPipelineStfnLambda",
      props.eksAgent2AOSPipelineStfnLambdaArn
    );
    const lambdaTask = new tasks.LambdaInvoke(
      this,
      `Set Ingestion Status to ACTIVE`,
      {
        lambdaFunction: eksAgent2AOSPipelineStfnLambda,
        outputPath: "$.Payload",
      }
    );
    // Step Functions Tasks
    const applogPipelineTable = Table.fromTableArn(
      this,
      "ApplogPipelineTable",
      props.applogPipelineTableArn
    );
    const applogIngestionTable = Table.fromTableArn(
      this,
      "ApplogIngestionTable",
      props.applogIngestionTableArn
    );

    //Applog Ingestion Status
    const appLogIngestionErrorStatus = this.updateStatus(
      "ApplogIngestion",
      applogIngestionTable,
      "ERROR"
    );
    const appLogIngestionInactiveStatus = this.updateStatus(
      "ApplogIngestion",
      applogIngestionTable,
      "INACTIVE"
    );
    //AppLog Pipeline Status
    const appLogPipelineActiveStatus = this.updateStatus(
      "ApplogPipeline",
      applogPipelineTable,
      "ACTIVE",
      {
        osHelperFnArn: tasks.DynamoAttributeValue.fromString(
          sfn.JsonPath.stringAt("$.result.outputs[0].OutputValue")
        ), // OSInitHelperFn
      }
    );
    const appLogPipelineErrorStatus = this.updateStatus(
      "ApplogPipeline",
      applogPipelineTable,
      "ERROR"
    );
    const appLogPipelineInactiveStatus = this.updateStatus(
      "ApplogPipeline",
      applogPipelineTable,
      "INACTIVE"
    );
    ...
    const logGroup = new logs.LogGroup(this, "ErrorLogGroup", {
      ...
      )}-SM-eks-cluster-agent-to-aos-error`,
    });

    // Role for state machine
    const LogHubAPIEKSAgent2AOSPipelineFlowSMRole = new iam.Role(
      this,
      "SMRole",
      {
    ...
    LogHubAPIEKSAgent2AOSPipelineFlowSMRole.addToPolicy(
    ...
    const pipeSM = new sfn.StateMachine(
      this,
      "EKSAgent2AOSPipelineFlowSMRole",
      {
        definition: cfnTask.next(checkStatus),
        role: LogHubAPIEKSAgent2AOSPipelineFlowSMRole,
        logs: {
