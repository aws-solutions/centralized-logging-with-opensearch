...
import { Construct, Duration, Aws, Fn } from '@aws-cdk/core';
...
export class CfnFlowStack extends Construct {
    ...
    constructor(scope: Construct, id: string, props: CfnFlowProps) {
        ...
        const cfnHandler = new lambda.Function(this, 'CfnHelper', {
            code: lambda.AssetCode.fromAsset(path.join(__dirname, '../../lambda/main/cfnHelper')),
            runtime: lambda.Runtime.PYTHON_3_9,
            handler: 'lambda_function.lambda_handler',
            timeout: Duration.seconds(60),
            memorySize: 128,
            environment: {
                DIST_OUTPUT_BUCKET: process.env.DIST_OUTPUT_BUCKET ? process.env.DIST_OUTPUT_BUCKET : 'aws-gcr-solutions',
        ...
        addCfnNagSuppressRules(cfnHandlerPolicy.node.defaultChild as iam.CfnPolicy, [
            {
                id: 'F4',
                reason: 'This policy requires releted actions in order to start/delete sub cloudformation stacks with many other services'
            },
            {
                id: 'W76',
        ...
        const sfnHandler = new lambda.Function(this, 'SfnHelper', {
            code: lambda.AssetCode.fromAsset(path.join(__dirname, '../../lambda/main/sfnHelper')),
            runtime: lambda.Runtime.PYTHON_3_9,
            handler: 'lambda_function.lambda_handler',
        ...
        addCfnNagSuppressRules(sfnHandlerPolicy.node.defaultChild as iam.CfnPolicy, [
            {
                id: 'W12',
        ...
        const stackCompleted = new sfn.Choice(this, 'In progress?')
            .when(
                sfn.Condition.stringMatches('$.result.stackStatus', '*_IN_PROGRESS'), wait)
            .otherwise(sfnNotifyTask)

        const stackFailed = new sfn.Choice(this, 'Failed?')
            .when(
                sfn.Condition.stringMatches('$.result.stackStatus', '*_IN_PROGRESS'), wait.next(cfnQueryTask.next(stackCompleted)))
            .otherwise(sfnNotifyTask)
        ...
        const logGroup = new logs.LogGroup(this, 'ErrorLogGroup', {
            logGroupName: `/aws/vendedlogs/states/${Fn.select(6, Fn.split(':', sfnHandler.functionArn))}-SM-cfn-error`
        });
