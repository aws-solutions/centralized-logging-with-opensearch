import {
  Stack, StackProps, CfnParameter, CfnParameterProps, Aws,
  Fn,
  Duration,
  IAspect,
  Aspects,
...
const GB = 1024;
const ECR_IMG_VERSION = "1.2.0";
const NLB_MIN_PORT = 500;
const NLB_MAX_PORT = 20000;

export interface SyslogConfigProps {
  portNum: CfnParameter;
  protocolType: CfnParameter;
  configS3Key: CfnParameter;
}
...
export class SyslogtoECSStack extends SolutionStack {
  ...
  constructor(scope: Construct, id: string, props?: StackProps) {
    ...
    this.setDescription(
      `(SO8025-sys) - Log Hub - Syslog Pipeline Template - Version ${VERSION}`
    );
    ...
    const ecsTaskRoleArn = this.newParam(`ECSTaskRoleArn`, {
      description: 'ECS Task Role Arn to run ECS task (Please make sure the role exists)',
      type: 'String'
    });
    ...
    const ecsSubnets = this.newParam('ECSSubnets', {
      description: 'Subnet IDs to run ECS task. Please provide two private subnets at least delimited by comma, e.g. subnet-97bfc4cd,subnet-7ad7de32',
      type: 'List<AWS::EC2::Subnet::Id>'
    });

    const configS3BucketName = this.newParam(`ConfigS3BucketName`, {
      description: 'S3 bucket to store agent config files (Please make sure the bucket exists)',
      type: 'String'
    });
    ...
    const ecsCluster = ecs.Cluster.fromClusterAttributes(this, 'LogHubCluster', {
      clusterName: ecsClusterName.valueAsString,
      vpc: vpc,
      securityGroups: []
    })

    const configS3Bucket = s3.Bucket.fromBucketName(this, 'ConfigS3Bucket', configS3BucketName.valueAsString)
    ...
    const syslogNlb = elbv2.NetworkLoadBalancer.fromNetworkLoadBalancerAttributes(this, 'SyslogNLB', {
      ...
      public renderContainerDefinition(_taskDefinition?: TaskDefinition): CfnTaskDefinition.ContainerDefinitionProperty {
        return Object.assign(super.renderContainerDefinition(_taskDefinition), {
          portMappings: [{
            ...
            protocol: protocolTable.findInMap(protocolType.valueAsString, 'protocolLowerCase'),
          }]
        })
      }
    }

    taskDefinition.defaultContainer = new MyContainerDefinition(service, `ServiceContainer`, {
      taskDefinition: service.taskDefinition,
      image: ecs.ContainerImage.fromRegistry(`public.ecr.aws/aws-gcr-solutions/log-hub-syslog:${ECR_IMG_VERSION}`),
