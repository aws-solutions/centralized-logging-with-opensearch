...
import { Stack, Construct, CfnParameter, CfnOutput, CfnResource, Duration, Fn, Aws, Aspects } from '@aws-cdk/core';
...
export class S3toKDSStack extends Stack {
    ...
    constructor(scope: Construct, id: string) {
        ...
        const kinesisStreamNameParam = new CfnParameter(this, 'kinesisStreamNameParam', {
            type: 'String',
            description: 'The Kinesis Data Stream Name.',
            allowedPattern: '.+',
        });
        this.addToParamLabels('KinesisStreamNameParam', kinesisStreamNameParam.logicalId)

        const failedLogBucketParam = new CfnParameter(this, 'failedLogBucketParam', {
            type: 'String',
            description: 'The s3 bucket will store the agent config file.',
            default: 'failed-log-bucket',
            allowedPattern: '.+',
        });
        this.addToParamLabels('FailedLogBucketParam', failedLogBucketParam.logicalId)
        ...
        const logAgentSubnetIdsParam = new CfnParameter(this, 'logAgentSubnetIdsParam', {
            type: 'List<AWS::EC2::Subnet::Id>',
            description: 'Select the Public Subnet of Log Hub Default VPC.',
        });
        this.addToParamLabels('LogAgentSubnetIdsParam', logAgentSubnetIdsParam.logicalId)

        const sourceLogBucketParam = new CfnParameter(this, 'sourceLogBucketParam', {
            type: 'String',
            description: 'The s3 bucket to store raw logs.',
            default: 'raw-log-bucket',
            allowedPattern: '.+',
        });
        this.addToParamLabels('SourceLogBucketParam', sourceLogBucketParam.logicalId)

        const sourceLogBucketPrefixParam = new CfnParameter(this, 'sourceLogBucketPrefixParam', {
            type: 'String',
            default: '',
            description: 'The prefix of the s3 bucket to store raw logs.',
        });
        this.addToParamLabels('SourceLogBucketPrefixParam', sourceLogBucketPrefixParam.logicalId)
        ...
        this.addToParamGroups('Pipeline Config', agentSourceIdParam.logicalId, kinesisStreamNameParam.logicalId, failedLogBucketParam.logicalId, sourceLogBucketParam.logicalId, sourceLogBucketPrefixParam.logicalId, defaultCmkArnParam.logicalId)
        this.addToParamGroups('Network Config', logAgentVpcIdParam.logicalId, logAgentSubnetIdsParam.logicalId)

        const defaultKMSCmk = kms.Key.fromKeyArn(this, 'SqsCMK', defaultCmkArnParam.valueAsString)
        ...
        const sourceLogBucket = s3.Bucket.fromBucketName(this, 'SourceLogBucket', sourceLogBucketParam.valueAsString);

        sourceLogBucket.addEventNotification(s3.EventType.OBJECT_CREATED, new s3n.SqsDestination(logEventQueue), {
        ...
        logAgentRole.addManagedPolicy(iam.ManagedPolicy.fromAwsManagedPolicyName('AmazonSSMManagedInstanceCore'));
