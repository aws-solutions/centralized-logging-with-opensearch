...
export class InitLambdaS3ObjectMigrationStack extends Construct {
  ...
  constructor(scope: Construct, id: string, props: InitLambdaS3ObjectMigrationProps) {
    super(scope, id);
    ...
    const cfnS3ObjectMigrationRole = this.S3ObjectMigrationRole.node.defaultChild as iam.CfnRole;
    cfnS3ObjectMigrationRole.overrideLogicalId("S3ObjectMigrationRole");

    const S3ObjectMigrationRWDDBPolicy = new iam.Policy(this, "S3ObjectMigrationRWDDBPolicy", {
      statements: [
    ...
    // Override the logical ID
    const cfnS3ObjectMigrationRWDDBPolicy = S3ObjectMigrationRWDDBPolicy.node.defaultChild as iam.CfnPolicy;
    cfnS3ObjectMigrationRWDDBPolicy.overrideLogicalId("S3ObjectMigrationRWDDBPolicy");

    const S3ObjectMigrationRWSQSPolicy = new iam.Policy(this, "S3ObjectMigrationRWSQSPolicy", {
      statements: [
        new iam.PolicyStatement({
          effect: iam.Effect.ALLOW,
          actions: [
    ...
    // Override the logical ID
    const cfnS3ObjectMigrationRWSQSPolicy = S3ObjectMigrationRWSQSPolicy.node.defaultChild as iam.CfnPolicy;
    cfnS3ObjectMigrationRWSQSPolicy.overrideLogicalId("S3ObjectMigrationRWSQSPolicy");

    this.S3ObjectMigrationRole.addManagedPolicy(iam.ManagedPolicy.fromAwsManagedPolicyName("service-role/AWSLambdaBasicExecutionRole"));
    this.S3ObjectMigrationRole.addManagedPolicy(iam.ManagedPolicy.fromAwsManagedPolicyName("service-role/AWSLambdaVPCAccessExecutionRole"));
    this.S3ObjectMigrationRole.addManagedPolicy(microBatchIAMStack.S3PublicAccessPolicy);
    this.S3ObjectMigrationRole.addManagedPolicy(microBatchIAMStack.KMSPublicAccessPolicy);
    ...
    this.S3ObjectMigration = new lambda.Function(this, "S3ObjectMigration", {
      code: lambda.AssetCode.fromAsset(
        path.join(__dirname, "../../../../../lambda/microbatch/s3_object_migration"),
        {
    ...
    // Override the logical ID
    const cfnS3ObjectMigration = this.S3ObjectMigration.node.defaultChild as lambda.CfnFunction;
    ...
    // Override the logical ID
    const cfnS3ObjectMerge = this.S3ObjectMerge.node.defaultChild as lambda.CfnFunction;
    cfnS3ObjectMerge.overrideLogicalId("S3ObjectMerge");

    this.S3ObjectMerge.node.addDependency(S3ObjectMigrationRWSQSPolicy);

    const S3ObjectMergeEventSource = new SqsEventSource(microBatchSQSStack.S3ObjectMergeQ, {
      batchSize: 1,
    });
