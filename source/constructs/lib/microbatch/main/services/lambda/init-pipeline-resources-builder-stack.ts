...
export class InitLambdaPipelineResourcesBuilderStack extends Construct {
  ...
  constructor(scope: Construct, id: string, props: InitLambdaPipelineResourcesBuilderProps) {
    super(scope, id);
    ...
    const PipelineResourcesBuilderPolicy = new iam.Policy(this, "PipelineResourcesBuilderPolicy", {
      statements: [
        ...
        new iam.PolicyStatement({
          ...
          resources: [
            ...
            microBatchIAMStack.SendTemplateEmailSNSPublicPolicy.managedPolicyArn
          ],
        }),
      ],
    });
    ...
    const cfnPipelineResourcesBuilderPolicy = PipelineResourcesBuilderPolicy.node.defaultChild as iam.CfnPolicy;
    cfnPipelineResourcesBuilderPolicy.overrideLogicalId("PipelineResourcesBuilderPolicy");

    this.PipelineResourcesBuilderSchedulePolicy = new iam.ManagedPolicy(this, "PipelineResourcesBuilderSchedulePolicy", {
      statements: [
        new iam.PolicyStatement({
          sid: 'EventBridge',
          effect: iam.Effect.ALLOW,
    ...
    // Override the logical ID
    const cfnPipelineResourcesBuilderSchedulePolicy = this.PipelineResourcesBuilderSchedulePolicy.node.defaultChild as iam.CfnManagedPolicy;
    cfnPipelineResourcesBuilderSchedulePolicy.overrideLogicalId("PipelineResourcesBuilderSchedulePolicy");

    this.PipelineResourcesBuilderRole = new iam.Role(this, "PipelineResourcesBuilderRole", {
      assumedBy: new iam.ServicePrincipal('lambda.amazonaws.com'),
    });

    // Override the logical ID
    const cfnPipelineResourcesBuilderRole = this.PipelineResourcesBuilderRole.node.defaultChild as iam.CfnRole;
    cfnPipelineResourcesBuilderRole.overrideLogicalId("PipelineResourcesBuilderRole");

    this.PipelineResourcesBuilderRole.addManagedPolicy(iam.ManagedPolicy.fromAwsManagedPolicyName("service-role/AWSLambdaBasicExecutionRole"));
    this.PipelineResourcesBuilderRole.addManagedPolicy(iam.ManagedPolicy.fromAwsManagedPolicyName("service-role/AWSLambdaVPCAccessExecutionRole"));
    this.PipelineResourcesBuilderRole.addManagedPolicy(microBatchIAMStack.GluePublicAccessPolicy);
    this.PipelineResourcesBuilderRole.addManagedPolicy(microBatchIAMStack.KMSPublicAccessPolicy);
    this.PipelineResourcesBuilderRole.addManagedPolicy(this.PipelineResourcesBuilderSchedulePolicy);
    this.PipelineResourcesBuilderRole.attachInlinePolicy(PipelineResourcesBuilderPolicy);

    // Create a lambda to handle all pipeline resources.
    this.PipelineResourcesBuilder = new lambda.Function(this, "PipelineResourcesBuilder", {
      code: lambda.AssetCode.fromAsset(
        path.join(__dirname, "../../../../../lambda/microbatch/pipeline_resources_builder"),
      ...
      layers: [microBatchLambdaLayerStack.microBatchLambdaUtilsLayer, microBatchLambdaLayerStack.microBatchLambdaBoto3Layer],
      environment: {
