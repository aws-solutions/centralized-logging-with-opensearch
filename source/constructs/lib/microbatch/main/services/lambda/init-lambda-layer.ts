...
export class InitLambdaLayerStack extends Construct {
  ...
  constructor(scope: Construct, id: string, props: InitLambdaLayerProps) {
    ...
    this.microBatchLambdaBoto3Layer = new lambda.LayerVersion(this, 'LambdaBoto3Layer', {
    ...
    const cfnMicroBatchLambdaBoto3Layer = this.microBatchLambdaBoto3Layer.node.defaultChild as lambda.CfnLayerVersion;
    cfnMicroBatchLambdaBoto3Layer.overrideLogicalId("LambdaBoto3Layer");

    this.microBatchLambdaPyarrowLayer = new lambda.LayerVersion(this, 'LambdaPyarrowLayer', {
      removalPolicy: RemovalPolicy.DESTROY,
      code: lambda.AssetCode.fromAsset(
        path.join(__dirname, "./layer"),
        {
    ...
    // Override the logical ID
    const cfnMicroBatchLambdaPyarrowLayer = this.microBatchLambdaPyarrowLayer.node.defaultChild as lambda.CfnLayerVersion;
    cfnMicroBatchLambdaPyarrowLayer.overrideLogicalId("LambdaPyarrowLayer");

    // Create a lambda layer with required python packages.
    this.microBatchLambdaUtilsLayer = new lambda.LayerVersion(this, 'LambdaUtilsLayer', {
      removalPolicy: RemovalPolicy.DESTROY,
      code: lambda.AssetCode.fromAsset(
        path.join(__dirname, "./layer"),
        {
