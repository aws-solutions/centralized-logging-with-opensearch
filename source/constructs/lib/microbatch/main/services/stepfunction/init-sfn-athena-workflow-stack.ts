...
export class InitSFNAthenaWorkflowStack extends Construct {
  ...
  constructor(scope: Construct, id: string, props: InitSFNAthenaWorkflowProps) {
    ...
    const AthenaWorkflowLambdaInvokePolicy = new iam.Policy(
      this,
    ...
    const cfnAthenaWorkflowLambdaInvokePolicy = AthenaWorkflowLambdaInvokePolicy
      .node.defaultChild as iam.CfnPolicy;
    cfnAthenaWorkflowLambdaInvokePolicy.overrideLogicalId(
      'AthenaWorkflowLambdaInvokePolicy'
    );
    ...
    this.AthenaWorkflowRole.addManagedPolicy(
    ...
    this.AthenaWorkflowRole.attachInlinePolicy(
      AthenaWorkflowLambdaInvokePolicy
    );

    // Override the logical ID
    const cfnAthenaWorkflowRole = this.AthenaWorkflowRole.node
    ...
    const writeQueryExecutionStatusToDDB = new tasks.LambdaInvoke(
      this,
      'Put Athena: StartQueryExecution logs to DynamoDB',
      {
        ...
        payload: sfn.TaskInput.fromObject({
          ...
          parameters: {
            'queryExecutionId.$':
              '$.athena.response.QueryExecution.QueryExecutionId',
          },
          extra: {
    ...
    const writeQueryExecutionFailedStatusToDDB = new tasks.LambdaInvoke(
      this,
      'Put Athena: StartQueryExecution failed logs to DynamoDB',
      {
        lambdaFunction: microBatchLambdaStack.ETLHelperStack.ETLHelper,
        payload: sfn.TaskInput.fromObject({
    ...
    startQueryExecution.addCatch(writeQueryExecutionFailedStatusToDDB, {
      resultPath: '$.errors.startQueryExecution',
    });
    ...
    // Override the logical ID
    const cfnAthenaWorkflow = this.AthenaWorkflow.node
