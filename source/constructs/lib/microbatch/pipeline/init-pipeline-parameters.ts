...
export class InitLogPipelineCfnParameters extends Construct {
  ...
  constructor(
    ...
    if (['fluent-bit', 's3'].includes(sourceType)) {
      ...
      if ( sourceType == 's3') {
        sourceDataFormatParameter = new CfnParameter(
          this,
          "sourceDataFormat",
          {
            type: "String",
    ...
    if ( addEnrichmentPlugins ) {
      enrichmentPluginsParameter = new CfnParameter(
        this,
        "enrichmentPlugins",
        {
          type: "String",
    ...
    if (['fluent-bit', 's3'].includes(sourceType)) {
      ...
      centralizedTableSchemaParameter.overrideLogicalId("centralizedTableSchema");
    ...
    this.sourceTableProperties = sourceTablePropertiesParameter?.valueAsString  ?? "{}";
    this.sourceSerializationProperties = sourceSerializationPropertiesParameter?.valueAsString  ?? "{}";
    ...
    this.centralizedBucketPrefix = centralizedBucketPrefixParameter.valueAsString;
    this.centralizedTableSchema = centralizedTableSchemaParameter?.valueAsString  ?? "{}";
    this.centralizedMetricsTableName = centralizedMetricsTableNameParameter?.valueAsString  ?? "";
    this.centralizedMetricsTableSchema = centralizedMetricsTableSchemaParameter?.valueAsString  ?? "{}";
