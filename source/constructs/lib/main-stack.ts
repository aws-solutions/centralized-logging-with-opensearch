...
import { Stack, StackProps, Construct, CfnParameter, CfnResource, CfnOutput, Fn, CfnMapping, Aws, RemovalPolicy, Duration } from '@aws-cdk/core';
...
export class MainStack extends Stack {
  ...
  constructor(scope: Construct, id: string, props?: MainProps) {
    ...
    if (this.authType === AuthType.OIDC) {
      ...
      this.addToParamGroups('OIDC Settings', oidcClientId.logicalId, oidcProvider.logicalId)
      this.addToParamGroups('Log Hub Console Settings', oidcCustomerDomain.logicalId, iamCertificateId.logicalId)
    ...
    if (props?.existingVpc) {
      ...
      const publicSubnetIds = new CfnParameter(this, 'publicSubnets', {
        description: 'Public Subnet IDs in seleted VPC. Please provide two subnets at least delimited by comma, e.g. subnet-97bfc4cd,subnet-7ad7de32',
      ...
      const privateSubnetIds = new CfnParameter(this, 'privateSubnets', {
        description: 'Private Subnet IDs in seleted VPC. Please provide two subnets at least delimited by comma, e.g. subnet-97bfc4cd,subnet-7ad7de32',
        type: 'List<AWS::EC2::Subnet::Id>'
      })
      privateSubnetIds.overrideLogicalId('privateSubnets')
      this.addToParamLabels('Private Subnet IDs', privateSubnetIds.logicalId)
      this.addToParamGroups('Existing VPC Info', vpcId.logicalId, publicSubnetIds.logicalId, privateSubnetIds.logicalId)
    ...
    const elbRootAccountArnTable = new CfnMapping(this, 'ELBRootAccountArnTable', {
      mapping: {
        ...
        'ca-central-1': { elbRootAccountArn: 'arn:aws:iam::985666609251:root' },
        'eu-central-1': { elbRootAccountArn: 'arn:aws:iam::054676820928:root' },
        ...
        'ap-northeast-1': { elbRootAccountArn: 'arn:aws:iam::582318560864:root' },
        'ap-northeast-2': { elbRootAccountArn: 'arn:aws:iam::600734575887:root' },
        'ap-northeast-3': { elbRootAccountArn: 'arn:aws:iam::383597477331:root' },
        'ap-southeast-1': { elbRootAccountArn: 'arn:aws:iam::114774131450:root' },
        'ap-southeast-2': { elbRootAccountArn: 'arn:aws:iam::783225319266:root' },
        'ap-southeast-3': { elbRootAccountArn: 'arn:aws:iam::589379963580:root' },
        ...
        'cn-north-1': { elbRootAccountArn: 'arn:aws-cn:iam::638102146993:root' },
        'cn-northwest-1': { elbRootAccountArn: 'arn:aws-cn:iam::037604701340:root' },
      }
    });
    ...
    loggingBucket.addToResourcePolicy(
      new iam.PolicyStatement({
        ...
        principals: [
          new iam.ArnPrincipal(elbRootAccountArnTable.findInMap(Aws.REGION, 'elbRootAccountArn')),
        ]
      })
    );
    loggingBucket.addToResourcePolicy(
    ...
    const instanceMetaStack = new InstanceMetaStack(this, `${solutionName}InstanceMetaAPI`, {
      graphqlApi: apiStack.graphqlApi,
    });

    // Create the Instance Group Appsync API stack
    const instanceGroupStack = new InstanceGroupStack(this, `${solutionName}InstanceGroupAPI`, {
      graphqlApi: apiStack.graphqlApi,
    ...
    const logSourceStack = new LogSourceStack(this, `${solutionName}LogSourceAPI`, {
      graphqlApi: apiStack.graphqlApi,
    });

    // Create the App Pipeline Appsync API stack
    const appPipelineStack = new AppPipelineStack(this, `${solutionName}AppPipelineAPI`, {
      graphqlApi: apiStack.graphqlApi,
      cfnFlowSMArn: cfnFlow.stateMachineArn,
    });

    // Create the Logging Conf Appsync API stack
    const eksClusterStack = new EKSClusterStack(this, `${solutionName}EKSClusterStack`, {
      graphqlApi: apiStack.graphqlApi,
      appLogIngestionTable: appPipelineStack.appLogIngestionTable,
      aosDomainTable: apiStack.clusterTable,
      eksClusterLogSourceTable: logSourceStack.eksClusterLogSourceTable
    });

    // Create the Logging Ingestion Appsync API stack
    const appLogIngestionStack = new AppLogIngestionStack(this, `${solutionName}AppLogIngestionAPI`, {
      cfnFlowSMArn: cfnFlow.stateMachineArn,
      graphqlApi: apiStack.graphqlApi,
      ...
      appPipelineTable: appPipelineStack.appPipelineTable,
      appLogIngestionTable: appPipelineStack.appLogIngestionTable,
    ...
    appLogIngestionStack.node.addDependency(sqsCMKKey, instanceGroupStack, instanceMetaStack, logConfStack, appPipelineStack)
