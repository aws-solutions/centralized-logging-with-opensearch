...
def sqs_exception_handler(err):
    logging.warning(err)
    if err.response["Error"]["Code"] != "AWS.SimpleQueueService.NonExistentQueue":
        raise err


@retry_with_backoff(ClientError, exception_handler=sqs_exception_handler, retries=5)
def send_message_batch(iterable, queue_url):
    entries = [
        {"Id": str(index), "MessageBody": json.dumps(each)}
        for index, each in enumerate(iterable)
    ]
    ...
    if "Failed" in res:
        for failed in res["Failed"]:
            ...
            logging.error(
                f"Failed to send message: {failed['Message']}, error code: {failed['Code']}"
            )


def send_message_batch_dry_run(iterable, _):
            ...
            "Records": [
                {
                    ...
                    "s3": {
                        ...
                        "bucket": {
                            "name": BUCKET_NAME,
                            "arn": f"arn:{AWS_PARTITION}:s3:::{BUCKET_NAME}",
                        },
                        "object": {
                            "key": each["Key"],
                            "size": each["Size"],
...
if __name__ == "__main__":
    ...
    try:
        for batch in batch_iter(it, batch_size=BATCH_SIZE):
            if DRY_RUN:
                send_message_batch_dry_run(batch, QUEUE_URL)
            else:
                send_message_batch(batch, QUEUE_URL)
    finally:
        t.stop()
        t.join(30)
